...
async fn create_github_issue(
    ...
    let body = format!(
        ...
        **Error Hash:** ERROR_HASH:{error_hash}\n\n\
        ...
        ```\n{error_message}\n```\n\n\
        ---\n\
        *This issue was automatically generated by the sensibleql-tests runner.*"
    );

    let labels = vec![
        "bug".to_string(),
        "automated".to_string(),
        "sensibleql-tests".to_string(),
    ];
...
async fn main() -> Result<()> {
    let matches = ClapCommand::new("queries-test")
        .about("Process nexus test directories")
        .arg(
            Arg::new("test_name")
    ...
    match nexus_check {
        Ok(output) => {
            if output.status.success() {
                println!(
                    "DEBUG: Nexus already available: {}",
                    String::from_utf8_lossy(&output.stdout)
                );
            } else {
                println!("DEBUG: Nexus not available or failed version check");
            }
        }
        Err(e) => {
            println!("DEBUG: Nexus command not found: {e}");
        }
    }
    ...
    if !output.status.success() {
        ...
        // Check if nexus is available after build
        let nexus_check_after = Command::new("nexus").arg("--version").output();

        match nexus_check_after {
            Ok(output) => {
                if output.status.success() {
                    println!(
                        "DEBUG: Nexus available after build: {}",
                        String::from_utf8_lossy(&output.stdout)
                    );
                } else {
                    println!("DEBUG: Nexus still not available after build");
                    println!(
                        "DEBUG: Nexus version check stderr: {}",
                        String::from_utf8_lossy(&output.stderr)
                    );
                }
            }
            Err(e) => {
                println!("DEBUG: Nexus command still not found after build: {e}");
            }
        }
    }
...
async fn process_test_directory(
    ...
    let temp_dir = env::temp_dir().join(format!("nexus_temp_{test_name}"));
    if temp_dir.exists() {
    ...
    // Copy all project crates and dependencies (excluding sensibleql-tests to avoid conflicts)
    ...
    // Copy root Cargo.toml and Cargo.lock, but remove sensibleql-tests from workspace
    ...
    let output = Command::new("nexus")
        ...
        .output()
        .context("Failed to execute nexus compile command")?;

    println!(
        "DEBUG: Nexus compile output: {}",
        String::from_utf8_lossy(&output.stdout)
    );
    println!(
        "DEBUG: Nexus compile stderr: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    if !output.status.success() {
        ...
        if let Some(config) = github_config {
            println!("DEBUG: Nexus compilation failed in parallel mode, creating GitHub issue...");
