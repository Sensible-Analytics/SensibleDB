...
use nexus_db::nexus_gateway::mcp::mcp::{MCPHandlerFn, MCPHandlerSubmission};
use nexus_db::nexus_gateway::{
...
fn main() {
    ...
    let path = match std::env::var("NEXUS_DATA_DIR") {
        Ok(val) => std::path::PathBuf::from(val).join("user"),
        Err(_) => {
            println!("NEXUS_DATA_DIR not set, using default");
            let home = dirs::home_dir().expect("Could not retrieve home directory");
            home.join(".nexus/user")
        }
    };

    let port = match std::env::var("NEXUS_PORT") {
        Ok(val) => val
            .parse::<u16>()
            .expect("NEXUS_PORT must be a valid port number"),
        Err(_) => 6969,
    };
