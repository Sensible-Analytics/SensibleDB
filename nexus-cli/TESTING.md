...
For each of these, make sure you're in the nexus-cli directory.
...
- `nexus init` with default settings; check nexus.toml created with empty template and ./db/ queries path
...
- `nexus init` in directory with existing nexus.toml; verify appropriate error message
...
- `nexus init --cloud` with cloud instance; verify cloud instance configured in nexus.toml
...
- `nexus sync` in a workspace without nexus.toml; ensure standard and enterprise clusters are selectable
