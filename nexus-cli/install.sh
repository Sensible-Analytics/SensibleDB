...
# Nexus CLI Installer
# Cross-platform installer for Nexus CLI

readonly REPO="NexusDB/nexus-db"
...
setup_path() {
    ...
    if [[ -f "$shell_config" ]] && ! grep -Fq "$path_line" "$shell_config"; then
        echo "" >> "$shell_config"
        echo "# Added by Nexus CLI installer" >> "$shell_config"
...
check_docker_desktop() {
    ...
    log_info "Nexus CLI requires Docker to be running for local development"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if brew install --cask docker; then
            ...
            log_info "You'll need to complete the Docker Desktop setup before using Nexus CLI"
        else
        ...
        log_warn "IMPORTANT: You need to have the Docker daemon running to use Nexus CLI"
