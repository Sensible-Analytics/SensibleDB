...
pub enum ConfigError {
    ...
    #[error("failed to read nexus.toml at {path}: {source}")]
    ReadNexusConfig {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to parse nexus.toml at {path}: {source}")]
    ParseNexusConfig {
        path: PathBuf,
        #[source]
        source: toml::de::Error,
    },
    #[error("failed to serialize nexus.toml: {source}")]
    SerializeNexusConfig {
        #[source]
        source: toml::ser::Error,
    },
    #[error("failed to write nexus.toml at {path}: {source}")]
    WriteNexusConfig {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("instance '{name}' not found in nexus.toml")]
    ...
    pub fn to_cli_error(&self) -> CliError {
        match self {
            ...
            ConfigError::ReadNexusConfig { path, source } => {
                CliError::new(format!("failed to read nexus.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ConfigError::ParseNexusConfig { path, source } => {
                CliError::new(format!("failed to parse nexus.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ConfigError::SerializeNexusConfig { source } => {
                CliError::new("failed to serialize nexus.toml").with_caused_by(source.to_string())
            }
            ConfigError::WriteNexusConfig { path, source } => {
                CliError::new(format!("failed to write nexus.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ...
            ConfigError::InstanceNotFound { name } => {
                CliError::new(format!("instance '{}' not found in nexus.toml", name))
            }
        }
    }
}
...
impl From<toml::de::Error> for CliError {
    fn from(err: toml::de::Error) -> Self {
        CliError::new("failed to parse TOML configuration")
            .with_caused_by(err.to_string())
            .with_hint("check the nexus.toml file for syntax errors")
    }
}
...
pub fn cloud_error<S: Into<String>>(message: S) -> CliError {
    CliError::new(message).with_hint("run `nexus auth login` to authenticate with Nexus Cloud")
}
