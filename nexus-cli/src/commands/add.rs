    ...
    let deployment_type = match deployment_type {
        ...
        None => {
            return Err(eyre::eyre!(
                "No deployment type specified. Run 'nexus add' in an interactive terminal or specify a deployment type:\n  \
                nexus add local\n  \
                nexus add cloud\n  \
                nexus add ecr\n  \
                nexus add fly"
...
async fn run_add_inner(
            ...
            .contains_key(&instance_name)
    {
        return Err(project_error(format!(
            "Instance '{instance_name}' already exists in nexus.toml"
        ))
    ...
    let config_path = project_context.root.join("nexus.toml");
    cleanup_tracker.backup_config(&project_context.config, config_path.clone());

    // Determine instance type

    match deployment_type {
        CloudDeploymentTypeCommand::Nexus { .. } => {
            ...
            if flow_result.resolved_project_name != project_context.config.project.name {
                crate::output::info(&format!(
                    "Updating project name in nexus.toml to '{}' to match cloud project.",
            ...
            Step::verbose_substep("Nexus Cloud configuration saved");
        }
        CloudDeploymentTypeCommand::Ecr { .. } => {
            ...
            // Update nexus.toml with cloud config
            project_context
                .config
                .cloud
    ...
    print_instructions(
        "Next steps:",
        &[
            &format!("Run 'nexus build {instance_name}' to compile your project for this instance"),
            &format!("Run 'nexus push {instance_name}' to start the '{instance_name}' instance"),
        ],
    );
