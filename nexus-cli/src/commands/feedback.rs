//! Send feedback to the Nexus team via GitHub issues.
...
pub async fn run(message: Option<String>) -> Result<()> {
    let (feedback_type, feedback_message) = if let Some(msg) = message {
        ...
        if !prompts::is_interactive() {
            return Err(eyre!(
                "No feedback message provided. Run 'nexus feedback \"your message\"' or run in an interactive terminal."
            ));
        }

        prompts::intro("nexus feedback", Some("Submit feedback for Nexus"))?;
