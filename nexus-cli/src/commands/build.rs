...
use crate::utils::{
    ...
    nexusc_utils::{collect_hx_contents, collect_hx_files},
...
use nexus_db::{
    nexus_engine::traversal_core::config::Config,
    nexusc::{
        analyzer::analyze,
    ...
    let instance_name = match instance_name {
        ...
        None if prompts::is_interactive() => {
            ...
            prompts::intro(
                "nexus build",
                Some(
                    "This will build your selected instance based on the configuration in nexus.toml.",
                ),
            )?;
...
async fn create_nexus_cache(repo_cache: &std::path::Path) -> Result<()> {
    Step::verbose_substep("Caching Nexus repository (first time setup)...");
...
async fn update_nexus_cache(repo_cache: &std::path::Path) -> Result<()> {
    Step::verbose_substep("Updating Nexus repository cache...");
...
fn create_git_cache(repo_cache: &std::path::Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        let error = crate::errors::CliError::new("failed to clone Nexus repository")
...
fn update_git_cache(repo_cache: &std::path::Path) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(eyre::eyre!(
            "Failed to update Nexus repository:\n{}",
            stderr
        ));
    }
...
pub(crate) async fn compile_project(
    ...
    let instance_workspace = project.instance_workspace(instance_name);
    let nexus_container_dir = instance_workspace.join("nexus-container");
    let src_dir = nexus_container_dir.join("src");

    // Create the directories
    fs::create_dir_all(&src_dir)?;

    // Generate config.hx.json from nexus.toml
...
fn build_binary_using_cargo(
    ...
    // <path-to-.nexus>/<instance_name>/nexus-repo-copy/nexus-container/
