...
use crate::utils::nexusc_utils::{collect_hx_files, generate_content};
...
const DEFAULT_CLOUD_AUTHORITY: &str = "cloud.nexus-db.com";
...
fn build_standard_deploy_payload(
    ...
    nexus_toml_content: Option<String>,
...
impl<'a> NexusManager<'a> {
    ...
    pub(crate) async fn deploy(
        ...
        // Optionally load config from nexus.toml or legacy config.hx.json
        let nexus_toml_path = path.join("nexus.toml");
        let config_hx_path = path.join("config.hx.json");
        let schema_path = path.join("schema.hx");

        let _config: Option<Config> = if nexus_toml_path.exists() {
            // v2 format: nexus.toml (config is already loaded in self.project)
            None
        ...
        // get cluster information from nexus.toml
        ...
        let nexus_toml_content = {
            ...
            match toml::to_string_pretty(&pruned) {
                Ok(s) => Some(s),
                Err(e) => {
                    output::warning(&format!("Failed to serialize pruned nexus.toml: {}", e));
                    None
                }
            }
        };
    ...
    pub(crate) async fn deploy_by_cluster_id(
        ...
        Err(eyre!(
            "Cluster '{}' is not configured in nexus.toml. Run 'nexus sync' to refresh cluster metadata, then retry syncing cluster '{}'.",
            cluster_id,
            cluster_name_hint
        ))
    }
    ...
    pub(crate) async fn deploy_enterprise_by_cluster_id(
        ...
        Err(eyre!(
            "Enterprise cluster '{}' is not configured in nexus.toml. Run 'nexus sync' to refresh cluster metadata, then retry syncing cluster '{}'.",
            cluster_id,
            cluster_name_hint
        ))
    }
...
mod tests {
    ...
    fn build_standard_deploy_payload_includes_runtime_config_and_build_mode() {
        ...
        assert!(payload["nexus_toml"].is_string());
    }
}
