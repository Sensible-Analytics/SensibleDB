...
impl<'a> EcrManager<'a> {
    ...
    fn repository_name(&self, instance_name: &str) -> String {
        format!("nexus-{}-{instance_name}", self.project.config.project.name)
    }
    ...
    pub async fn load_config(&self, instance_name: &str) -> Result<EcrConfig> {
        ...
        if !config_path.exists() {
            return Err(eyre!(
                "ECR configuration not found at {}. Run 'nexus init ecr' first.",
                config_path.display()
            ));
        }
    ...
    pub async fn get_project_status(&self) -> Result<Vec<EcrRepositoryStatus>> {
        ...
        let project_prefix = format!("nexus-{}-", self.project.config.project.name);
