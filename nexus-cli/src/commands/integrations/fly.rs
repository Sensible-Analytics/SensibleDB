...
impl<'a> FlyManager<'a> {
    ...
    pub async fn new(project: &'a ProjectContext, auth_type: FlyAuthType) -> Result<Self> {
        let auth = match auth_type {
            FlyAuthType::ApiKey => {
                let env_path = project.nexus_dir.join("nexus.env");
    ...
    pub async fn init_app(&self, instance_name: &str, config: &FlyInstanceConfig) -> Result<()> {
        ...
        match &self.auth {
            ...
            FlyAuth::Cli => {
                ...
                let nexus_dir_path = self.project.instance_workspace(instance_name);
                ...
                let mut launch_args = vec![
                    ...
                    "--no-deploy",
                    "--path",
                    nexus_dir_path.to_str().ok_or_else(|| {
                        eyre!(
                            "cannot convert nexus instance workspace to string: {nexus_dir_path:?}"
                        )
                    })?,
                ];
