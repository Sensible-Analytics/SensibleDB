...
async fn prepare_env_vars_from_context(
    ...
    match ProjectContext::find_and_load(None) {
        ...
        Err(_) => {
            ...
            output::info(&format!(
                "No nexus.toml found, using default connection (localhost:{DEFAULT_NEXUS_PORT})"
            ));
    ...
    match instance {
        ...
        None => {
            ...
            if instances.is_empty() {
                return Err(eyre!("No instances configured in nexus.toml"));
            }
...
async fn check_dev_mode_requirement(
    ...
    if !prompts::is_interactive() {
        return Err(eyre!(
            ...
            Redeploy with 'nexus push {} --dev' or update build_mode to 'dev' in nexus.toml.",
            instance_name,
            instance_name
        ));
    }
    ...
    if !should_redeploy {
        return Err(eyre!(
            "Dashboard requires dev mode. Update build_mode to 'dev' in nexus.toml or use --host flag to connect directly."
        ));
    }
    ...
    if instance_config.is_local() {
        ...
        let config_path = project.root.join("nexus.toml");
        ...
        output::info("Using a one-time dev override for this redeploy; nexus.toml is unchanged.");
...
async fn prepare_environment_vars(
    ...
    if instance_config.is_local() {
        ...
        // Add cluster ID for Nexus Cloud instances
