...
async fn login() -> Result<()> {
    output::info("Logging into Nexus Cloud");

    let home = dirs::home_dir().ok_or_eyre("Cannot find home directory")?;
    let config_path = home.join(".nexus");
    ...
    output::info("Your credentials are stored in ~/.nexus/credentials");

    Ok(())
}

async fn logout() -> Result<()> {
    output::info("Logging out of Nexus Cloud");
...
/// Check that the user is authenticated with Nexus Cloud.
...
pub async fn require_auth() -> Result<Credentials> {
    ...
    output::warning("Not authenticated with Nexus Cloud");
