...
async fn run_init_inner(
    ...
    // Create default nexus.toml with custom queries path
        ...
        Some(deployment) => {
            match deployment {
                CloudDeploymentTypeCommand::Nexus { name, .. } => {
                    ...
                    config.save_to_file(&config_path)?;
                    Step::verbose_substep("Nexus Cloud configuration saved to nexus.toml");
                }
...
"#;
    ...
    let gitignore = [".nexus/", "target/", "*.log"];
