...
async fn check_instance(
    ...
    let nexus_container_dir = instance_workspace.join("nexus-repo-copy/nexus-container");
    let cargo_output = run_cargo_check(&nexus_container_dir)?;
...
async fn check_all_instances(
    ...
    if instances.is_empty() {
        return Err(eyre::eyre!(
            "No instances found in nexus.toml. Add at least one instance to check."
        ));
    }
...
fn run_cargo_check(nexus_container_dir: &Path) -> Result<CargoCheckOutput> {
    let output = Command::new("cargo")
        .arg("check")
        .arg("--color=never") // Disable color codes for cleaner output
        .current_dir(nexus_container_dir)
        .output()
