...
pub async fn run(
    ...
    let nexus_toml_path = project_dir.join("nexus.toml");
    if nexus_toml_path.exists() {
        return Err(project_error("nexus.toml already exists in this directory")
...
fn show_migration_plan(ctx: &MigrationContext) -> Result<()> {
    ...
    println!("  {}: nexus.toml", "Create file".bright_white().bold());

    if !ctx.no_backup {
        println!(
            "  {}: .nexus/v1-backup/config.hx.json",
    ...
    let v1_nexus_dir = home_dir.join(".nexus");
    if v1_nexus_dir.exists() {
        let v2_marker = v1_nexus_dir.join(".v2");
        if v2_marker.exists() {
            println!(
                "  {}: ~/.nexus directory already migrated to v2",
                "Already migrated".bright_white().bold()
            );
        } else {
            println!(
                "  {}: ~/.nexus → ~/.nexus-v1-backup",
                "Create backup".bright_white().bold()
            );
            ...
            println!(
                "  {}: Remove all except ~/.nexus/credentials and ~/.nexus/repo",
            ...
            println!(
                "  {}: Create ~/.nexus/.v2 marker file",
                "Mark migrated".bright_white().bold()
            );
        }
    } else {
        println!(
            "  {}: ~/.nexus directory not found",
...
fn create_backup(ctx: &MigrationContext) -> Result<()> {
    ...
    // Create .nexus/v1-backup directory
    let backup_dir = ctx.project_dir.join(".nexus/v1-backup");
    ...
    output::success("Created backup: .nexus/v1-backup/config.hx.json");
    Ok(())
}
...
fn create_v2_config(ctx: &MigrationContext) -> Result<()> {
    output::info("Creating nexus.toml configuration");
    ...
    // Save to file
    let config_path = ctx.project_dir.join("nexus.toml");
    nexus_config
        .save_to_file(&config_path)
        .map_err(|e| CliError::new("Failed to create nexus.toml").with_caused_by(e.to_string()))?;

    output::success("Created nexus.toml configuration");
    Ok(())
}

fn provide_post_migration_guidance(ctx: &MigrationContext) -> Result<()> {
    // Check if user has Nexus Cloud credentials
    ...
    if has_cloud_credentials {
        output::info("You're authenticated with Nexus Cloud");
        ...
        print_instructions(
            ...
            &[
                ...
                "Run 'nexus push production' to deploy to Nexus Cloud",
            ],
        );
    } else {
        output::info("Ready for Nexus Cloud?");
        output::info("Take your project to production with managed infrastructure");
        print_instructions(
            "To get started with Nexus Cloud:",
            &[
                "Run 'nexus auth login' to authenticate with Nexus Cloud",
...
fn migrate_home_directory(_ctx: &MigrationContext) -> Result<()> {
    output::info("Migrating ~/.nexus directory");
    ...
    if !v1_nexus_dir.exists() {
        output::info("No ~/.nexus directory found, skipping home migration");
        return Ok(());
    }
    ...
    let v2_marker = v1_nexus_dir.join(".v2");
    if v2_marker.exists() {
        output::info("~/.nexus directory already migrated to v2, skipping home migration");
        return Ok(());
    }

    // Create backup of the entire .nexus directory
    let backup_dir = home_dir.join(".nexus-v1-backup");
    ...
    crate::utils::copy_dir_recursively(&v1_nexus_dir, &backup_dir).map_err(|e| {
        CliError::new("Failed to backup ~/.nexus directory").with_caused_by(e.to_string())
    })?;

    output::success("Created backup: ~/.nexus-v1-backup");
    ...
    let temp_credentials = if credentials_path.exists() {
        let temp_path = home_dir.join(".nexus-credentials-temp");
    ...
    let temp_repo = if repo_path.exists() {
        let temp_path = home_dir.join(".nexus-repo-temp");
    ...
    fs::remove_dir_all(&v1_nexus_dir).map_err(|e| {
        CliError::new("Failed to remove ~/.nexus directory").with_caused_by(e.to_string())
    })?;

    // Recreate .nexus directory
    fs::create_dir_all(&v1_nexus_dir).map_err(|e| {
        CliError::new("Failed to recreate ~/.nexus directory").with_caused_by(e.to_string())
    })?;
    ...
    if let Some(temp_creds) = temp_credentials {
        ...
        output::info("Preserved ~/.nexus/credentials");
    }
    ...
    if let Some(temp_repo) = temp_repo {
        ...
        output::info("Preserved ~/.nexus/repo");
    }
    ...
    output::success("Cleaned up ~/.nexus directory, preserving credentials and repo");
    Ok(())
}
...
fn check_cloud_credentials() -> bool {
    ...
    let credentials_path = home.join(".nexus").join("credentials");
