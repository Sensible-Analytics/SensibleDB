...
async fn fetch_enterprise_sync_response_with_remote_empty_fallback(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| eyre!("Failed to connect to Nexus Cloud: {}", e))?;
...
fn parse_and_sanitize_remote_config(
    ...
    let mut remote_config = match toml::from_str::<crate::config::NexusConfig>(remote_toml) {
        ...
        Err(e) => {
            print_warning(&format!(
                "Ignoring remote nexus.toml from {}: failed to parse ({})",
                source, e
            ));
            return None;
        }
...
fn update_project_queries_path_in_nexus_toml(
    ...
    let nexus_toml_path = project_root.join("nexus.toml");
    let mut config = NexusConfig::from_file(&nexus_toml_path)
        .map_err(|e| eyre!("Failed to load nexus.toml for queries path update: {}", e))?;

    config.project.queries = sanitize_relative_path(queries_path)?;
    config
        .save_to_file(&nexus_toml_path)
        .map_err(|e| eyre!("Failed to update queries path in nexus.toml: {}", e))?;
...
async fn reconcile_project_config_from_cloud(
    ...
    let mut config = if nexus_toml_path.exists() {
        NexusConfig::from_file(&nexus_toml_path)
            .map_err(|e| eyre!("Failed to load nexus.toml: {}", e))?
    } else {
        NexusConfig {
    ...
    for cluster in &project_clusters.enterprise {
        ...
        {
            print_warning(&format!(
                "Enterprise cluster '{}' uses different gateway ({}) and DB ({}) counts; nexus.toml stores these as min_instances/max_instances for compatibility.",
    ...
    config
        .save_to_file(&nexus_toml_path)
        .map_err(|e| eyre!("Failed to write nexus.toml: {}", e))?;
...
async fn run_project_sync_flow(project: &ProjectContext, assume_yes: bool) -> Result<()> {
    prompts::intro(
        "nexus sync",
        Some(&format!(
            "Using project '{}' from nexus.toml. Select a cluster to sync from.",
    ...
    .await?;
    crate::output::info(
        "Updated nexus.toml with canonical project and cluster metadata from Nexus Cloud.",
    );
...
pub async fn run(instance_name: Option<String>, assume_yes: bool) -> Result<()> {
    ...
    if let Some(instance_name) = instance_name {
        let project = project.ok_or_else(|| {
            eyre!("No nexus.toml found. Run 'nexus init' to create a project first.")
        })?;
...
async fn run_workspace_sync_flow() -> Result<()> {
    prompts::intro(
        "nexus sync",
        Some("No nexus.toml found. Select a workspace and cluster to sync from."),
    )?;
...
async fn sync_from_cluster_id(api_key: &str, cluster_id: &str) -> Result<()> {
    ...
    .await?;
    files_written += 1;
    Step::verbose_substep("  Wrote nexus.toml (canonical cloud metadata)");
...
async fn pull_from_cloud_instance(
    ...
    match instance_config {
        ...
        InstanceInfo::FlyIo(_) => Err(eyre!(
            "Sync is only supported for Nexus Cloud instances, not Fly.io deployments"
        )),
        InstanceInfo::Ecr(_) => Err(eyre!(
            "Sync is only supported for Nexus Cloud instances, not ECR deployments"
        )),
