...
pub async fn run(
    ...
    let instance_name = match instance_name {
        ...
        None if prompts::is_interactive() => {
            ...
            prompts::intro(
                "nexus push",
                Some(
                    "This will build and redeploy your selected instance based on the configuration in nexus.toml.",
                ),
            )?;
    ...
    // Check auth early for Nexus Cloud / Enterprise instances
    if matches!(
        &instance_config,
        ...
        CloudConfig::Nexus(_) => {
            ...
            let build_mode_override = if dev {
                crate::output::warning(
                    "Using one-time dev build override for this deploy; nexus.toml build_mode is unchanged.",
                );
                Some(BuildMode::Dev)
            } else {
                None
