...
/// Global workspace configuration stored in ~/.nexus/config
...
impl WorkspaceConfig {
    ...
    pub fn config_path() -> Result<PathBuf, ConfigError> {
        ...
        Ok(home.join(".nexus").join("config"))
    }

    /// Load the workspace config from ~/.nexus/config
    ...
    /// Save the workspace config to ~/.nexus/config
...
impl NexusConfig {
    ...
    /// Returns tuples of (name, type_hint) e.g. ("dev", "local"), ("prod", "Nexus Cloud")
    pub fn list_instances_with_types(&self) -> Vec<(&String, &'static str)> {
        ...
        for (name, config) in &self.cloud {
            let type_hint = match config {
                CloudConfig::Nexus(_) => "Nexus Cloud",
