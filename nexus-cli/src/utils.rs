...
use nexus_db::nexusc::parser::types::HxFile;
use std::io::IsTerminal;
use std::{borrow::Cow, fs, path::Path};
use tokio::sync::oneshot;
use tokio::time::Duration;

const IGNORES: [&str; 3] = ["target", ".git", ".nexus"];
...
pub mod nexusc_utils {
    ...
    pub fn collect_hx_files(root: &Path, queries_dir: &Path) -> Result<Vec<std::fs::DirEntry>> {
        ...
        fn collect_from_dir(dir: &Path, files: &mut Vec<std::fs::DirEntry>) -> Result<()> {
            if dir.file_name().unwrap_or_default() == ".nexus" {
                return Ok(());
            }
    ...
    fn format_diagnostics(
        diagnostics: &[nexus_db::nexusc::analyzer::diagnostic::Diagnostic],
        src: &str,
        files: &[HxFile],
    ) -> String {
