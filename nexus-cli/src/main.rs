...
use eyre::Result;
use nexus_cli::{AuthAction, CloudDeploymentTypeCommand, DashboardAction, MetricsAction};
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize a new Nexus project with nexus.toml
    Init {
    ...
    Compile {
        /// Directory containing nexus.toml (defaults to current directory or project root)
        #[arg(short, long)]
        path: Option<String>,
    ...
    Push {
        ...
        instance: Option<String>,
        /// Use development profile for faster builds (Nexus Cloud only)
        #[arg(long)]
        dev: bool,
    },

    /// Sync .hx source files and config from a deployed Nexus Cloud instance
    Sync {
...
fn display_welcome(update_available: Option<String>) {
    ...
    print_command("nexus auth login", "Login to Nexus Cloud", use_color);
    ...
    if use_color {
        ...
        println!("  Docs: https://docs.nexus-db.com");
    }
    println!();
}
