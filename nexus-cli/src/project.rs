...
pub struct ProjectContext {
    ...
    /// The path to the .nexus directory (including ".nexus")
...
impl ProjectContext {
    ...
    pub fn find_and_load(start_dir: Option<&Path>) -> Result<Self, ProjectError> {
        ...
        let config_path = root.join("nexus.toml");
...
/// Find the project root by looking for nexus.toml file
fn find_project_root(start: &Path) -> Result<PathBuf, ProjectError> {
    let mut current = start.to_path_buf();

    loop {
        let config_path = current.join("nexus.toml");
...
pub fn get_nexus_cache_dir() -> Result<PathBuf> {
    // Allow override for testing - tests can set NEXUS_CACHE_DIR to use isolated directories
    if let Ok(override_dir) = std::env::var("NEXUS_CACHE_DIR") {
    ...
    // Check if this is a fresh installation (no .nexus directory exists)
