...
/// AWS/Nexus Cloud region options
...
pub async fn build_deployment_command(
    ...
    // Check auth early for Nexus Cloud instances before prompting for more details
    ...
    match deployment_type {
        ...
        DeploymentType::NexusCloud => {
            // Check auth early for Nexus Cloud instances
...
pub fn select_instance(instances: &[(&String, &str)]) -> Result<String> {
    if instances.is_empty() {
        return Err(eyre::eyre!(
            "No instances found in nexus.toml. Run 'nexus init' to create a project first."
        ));
    }
