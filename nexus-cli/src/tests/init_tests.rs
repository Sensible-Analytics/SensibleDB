...
/// Helper function to check if nexus.toml exists and is valid
fn assert_nexus_config_exists(project_dir: &PathBuf) {
    let config_path = project_dir.join("nexus.toml");
    assert!(
        config_path.exists(),
...
fn assert_project_structure(project_dir: &PathBuf, queries_path: &str) {
    ...
    assert!(
        gitignore_content.contains(".nexus/"),
        ".gitignore should contain .nexus/"
    );
}
...
async fn test_init_with_custom_queries_path() {
    ...
    let config_content =
        fs::read_to_string(project_path.join("nexus.toml")).expect("Failed to read config");
    assert!(
...
async fn test_init_fails_if_nexus_toml_exists() {
    ...
    // Create nexus.toml first
    fs::write(project_path.join("nexus.toml"), "[project]").expect("Failed to create nexus.toml");
    ...
    assert!(result.is_err(), "Init should fail if nexus.toml exists");
...
async fn test_init_appends_gitignore_with_guard_newline() {
    ...
    assert!(
        !gitignore_content.contains("node_modules.nexus/"),
...
async fn test_init_nexus_dir_is_created() {
    ...
    let nexus_dir = project_path.join(".nexus");
    assert!(nexus_dir.exists(), ".nexus directory should exist");
    assert!(nexus_dir.is_dir(), ".nexus should be a directory");
}
...
async fn test_init_multiple_times_in_different_dirs() {
    ...
    assert!(project1.join("nexus.toml").exists());
    assert!(project2.join("nexus.toml").exists());
}
