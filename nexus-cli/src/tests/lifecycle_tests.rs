...
#[tokio::test]
async fn test_start_fails_without_nexus_project() {
    ...
    assert!(
        result.is_err(),
        "Start should fail when not in a nexus project"
    );
    let error_msg = format!("{:?}", result.err().unwrap());
    assert!(
        error_msg.contains("not found") || error_msg.contains("nexus.toml"),
...
#[tokio::test]
async fn test_stop_fails_without_nexus_project() {
    let ctx = TestContext::new();
    // Don't set up any project
    ...
    assert!(
        result.is_err(),
        "Stop should fail when not in a nexus project"
    );
}
...
#[tokio::test]
async fn test_restart_fails_without_nexus_project() {
    let ctx = TestContext::new();
    // Don't set up any project
    ...
    assert!(
        result.is_err(),
        "Restart should fail when not in a nexus project"
    );
}
...
async fn test_status_with_multiple_instances() {
    ...
    let mut config = NexusConfig::default_config("multi-instance-project");
    config.local.insert(
        "staging".to_string(),
    ...
    config
        .save_to_file(&ctx.project_path.join("nexus.toml"))
        .expect("Failed to save config");
    fs::create_dir_all(ctx.project_path.join(".nexus")).expect("Failed to create .nexus");
...
async fn test_delete_fails_with_nonexistent_instance_dev() {
    ...
    // Create nexus.toml but clear all instances - this prevents the test from
    // walking up directories and finding another test's nexus.toml
...
#[tokio::test]
async fn test_project_context_finds_nexus_toml() {
    ...
    assert!(
        result.is_ok(),
        "ProjectContext should find nexus.toml: {:?}",
        result.err()
    );
...
#[tokio::test]
async fn test_project_context_fails_without_nexus_toml() {
    use crate::project::ProjectContext;

    let ctx = TestContext::new();
    // Don't create nexus.toml

    let result = ProjectContext::find_and_load(Some(&ctx.project_path));
    assert!(
        result.is_err(),
        "ProjectContext should fail without nexus.toml"
    );
}
