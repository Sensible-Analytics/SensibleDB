...
fn setup_test_project() -> (TempDir, PathBuf) {
    ...
    fs::create_dir_all(project_path.join(".nexus")).expect("Failed to create .nexus");
...
fn test_find_project_root_fails_without_config() {
    ...
    let result = ProjectContext::find_and_load(Some(&project_path));
    assert!(result.is_err(), "Should fail when no nexus.toml exists");
...
fn test_project_context_find_and_load() {
    ...
    assert_eq!(context.nexus_dir, project_path.join(".nexus"));
}
...
fn test_project_context_instance_workspace() {
    ...
    assert_eq!(workspace, project_path.join(".nexus/dev"));
}
...
fn test_project_context_volumes_dir() {
    ...
    assert_eq!(volumes_dir, project_path.join(".nexus/.volumes"));
}
...
fn test_project_context_instance_volume() {
    ...
    assert_eq!(volume, project_path.join(".nexus/.volumes/production"));
}
...
fn test_project_context_docker_compose_path() {
    ...
    assert_eq!(
        compose_path,
        project_path.join(".nexus/staging/docker-compose.yml")
    );
}
...
fn test_project_context_dockerfile_path() {
    ...
    assert_eq!(dockerfile_path, project_path.join(".nexus/dev/Dockerfile"));
}
...
fn test_project_context_container_dir() {
    ...
    assert_eq!(
        container_dir,
        project_path.join(".nexus/dev/nexus-container")
    );
}
...
#[test]
fn test_get_nexus_cache_dir_creates_directory() {
    use crate::tests::test_utils::TestContext;

    // Use TestContext to isolate the test from other tests
    let ctx = TestContext::new();

    // When NEXUS_CACHE_DIR is set (by TestContext), get_nexus_cache_dir should use it
    let result = get_nexus_cache_dir();
    assert!(result.is_ok(), "Should get nexus cache directory");

    let cache_dir = result.unwrap();
    assert!(
        cache_dir.exists(),
        "Cache directory should exist after calling get_nexus_cache_dir"
    );

    // With NEXUS_CACHE_DIR override, the path should be the cache_dir from TestContext
    assert_eq!(
        cache_dir, ctx.cache_dir,
        "Cache directory should use NEXUS_CACHE_DIR when set"
    );
}
...
fn test_find_project_root_stops_at_filesystem_root() {
    ...
    // Start from a directory that definitely won't have nexus.toml
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
...
#[test]
fn test_legacy_nexus_toml_without_project_id_still_loads() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let project_path = temp_dir.path().to_path_buf();
    let config_path = project_path.join("nexus.toml");
...
"#;
    ...
    let loaded = NexusConfig::from_file(&config_path).expect("Legacy config should load");
...
fn test_project_id_persists_round_trip() {
    ...
    let mut config = NexusConfig::default_config("persisted-project");
    config.project.id = Some("proj_12345".to_string());
    config
        .save_to_file(&config_path)
        .expect("Failed to save config with project id");

    let loaded = NexusConfig::from_file(&config_path).expect("Failed to reload saved config");
