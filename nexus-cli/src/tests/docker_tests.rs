...
/// Regression test: NEXUS_DATA_DIR must always be /data inside container
...
fn test_nexus_data_dir_uses_container_path() {
    ...
    let data_dir_var = env_vars
        .iter()
        .find(|v| v.starts_with("NEXUS_DATA_DIR="))
        .expect("NEXUS_DATA_DIR should be set");

    assert_eq!(
        data_dir_var, "NEXUS_DATA_DIR=/data",
        "NEXUS_DATA_DIR must use container path /data, not host path"
    );
}
...
fn test_docker_compose_volume_and_env() {
    ...
    assert!(
        compose.contains("NEXUS_DATA_DIR=/data"),
        "NEXUS_DATA_DIR in environment should be /data"
    );

    // Should NOT have host path in NEXUS_DATA_DIR
    assert!(
        !compose.contains("NEXUS_DATA_DIR=../.volumes"),
        "NEXUS_DATA_DIR should NOT contain host path"
    );
}

/// Verify host-side data_dir() still respects NEXUS_DATA_DIR for volume mount
#[test]
