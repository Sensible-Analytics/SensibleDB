use crate::{
    ...
    nexus_gateway::mcp::tools::{EdgeType, FilterTraversal, Order, ToolArgs, execute_query_chain},
    protocol::{Format, Request, Response},
    utils::id::v6_uuid,
};
use bumpalo::Bump;
use nexus_macros::mcp_handler;
...
pub fn search_keyword(input: &mut MCPToolInput) -> Result<Response, GraphError> {
    use crate::nexus_engine::traversal_core::ops::{bm25::search_bm25::SearchBM25Adapter, g::G};
...
pub fn search_vector_text(input: &mut MCPToolInput) -> Result<Response, GraphError> {
    use crate::nexus_engine::traversal_core::ops::{g::G, vectors::search::SearchVAdapter};
    use crate::nexus_gateway::embedding_providers::{EmbeddingModel, get_embedding_model};
    ...
    let results = G::new(storage, &txn, &arena)
        .search_v::<fn(&crate::nexus_engine::vector_core::vector::HVector, &heed3::RoTxn) -> bool, _>(
            query_vec_arena,
            k_value,
            label_arena,
            None
