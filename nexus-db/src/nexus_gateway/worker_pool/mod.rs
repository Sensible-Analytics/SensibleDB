...
impl WorkerPool {
    ...
    pub async fn process(&self, req: Request) -> Result<Response, NexusError> {
        ...
        channel.send_async((req, ret_tx)).await.map_err(|_| {
            ...
            NexusError::Graph(GraphError::New("Server is shutting down".into()))
        })?;
        ...
        ret_rx.await.unwrap_or_else(|_| {
            ...
            Err(NexusError::Graph(GraphError::New(
...
impl Worker {
    pub fn start(
        ...
        let handle = std::thread::spawn(move || {
            ...
            nexus_metrics::init_thread_local();
...
fn request_mapper(
    ...
    let res = res.unwrap_or(Err(NexusError::NotFound {
        ty: req_type,
        name: req_name.clone(),
    }));
