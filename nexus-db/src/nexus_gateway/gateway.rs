...
use crate::nexus_gateway::builtin::all_nodes_and_edges::nodes_edges_handler;
#[cfg(feature = "dev-instance")]
use crate::nexus_gateway::builtin::node_by_id::node_details_handler;
#[cfg(feature = "dev-instance")]
use crate::nexus_gateway::builtin::node_connections::node_connections_handler;
#[cfg(feature = "dev-instance")]
use crate::nexus_gateway::builtin::nodes_by_label::nodes_by_label_handler;
use crate::nexus_gateway::introspect_schema::introspect_schema_handler;
use crate::nexus_gateway::worker_pool::WorkerPool;
use crate::protocol;
use crate::{
    nexus_engine::traversal_core::{NexusGraphEngine, NexusGraphEngineOpts},
    nexus_gateway::mcp::mcp::MCPHandlerFn,
};
...
impl NexusGateway {
    pub fn new(
        ...
        let cluster_id = std::env::var("NEXUS_CLUSTER_ID").ok();
        NexusGateway {
    ...
    pub fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        trace!("Starting Nexus Gateway");

        let all_core_ids = core_affinity::get_core_ids().expect("unable to get core IDs");

        let all_core_ids = match std::env::var("NEXUS_CORES_OVERRIDE") {
            Ok(val) => {
                let override_count: usize = val
                    .parse()
                    .expect("NEXUS_CORES_OVERRIDE must be a valid number");
                if override_count > all_core_ids.len() {
                    warn!(
                        "NEXUS_CORES_OVERRIDE ({}) exceeds available cores ({}), using all cores",
                        override_count,
                        all_core_ids.len()
                    );
                    all_core_ids
        ...
        rt.block_on(async move {
            ...
            nexus_metrics::init_metrics_system();
            ...
            let shutdown_result = tokio::time::timeout(
                ...
                nexus_metrics::shutdown_metrics_system(),
            )
            .await;
...
async fn post_handler(
    ...
    #[cfg(feature = "api-key")]
    {
        use crate::nexus_gateway::key_verification::verify_key;
        if let Err(e) = verify_key(req.api_key.as_ref().unwrap()) {
            info!(?e, "Invalid API key");
            nexus_metrics::log_event(
                nexus_metrics::events::EventType::InvalidApiKey,
                nexus_metrics::events::InvalidApiKeyEvent {
    ...
    let input_body = if *nexus_metrics::METRICS_ENABLED {
        Some(req.body.clone())
    } else {
        None
    };
    ...
    match res {
        Ok(r) => {
            ...
            if !*nexus_metrics::METRICS_ENABLED {
                return r.into_response();
            }
            nexus_metrics::log_event(
                nexus_metrics::events::EventType::QuerySuccess,
                nexus_metrics::events::QuerySuccessEvent {
                    cluster_id: state.cluster_id.clone(),
                    query_name,
                    time_taken_usec: start_time.elapsed().as_micros() as u32,
                },
        ...
        Err(e) => {
            ...
            nexus_metrics::log_event(
                nexus_metrics::events::EventType::QueryError,
                nexus_metrics::events::QueryErrorEvent {
                    cluster_id: state.cluster_id.clone(),
                    query_name,
