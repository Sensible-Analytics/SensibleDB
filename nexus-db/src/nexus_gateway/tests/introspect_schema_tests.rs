...
use crate::{
    ...
    nexus_gateway::{gateway::AppState, introspect_schema::introspect_schema_handler},
};
use axum::body::Bytes;
...
async fn test_introspect_schema_invalid_api_key() {
    ...
    // verify_key returns NexusError::InvalidApiKey which converts to 403 Forbidden
...
async fn test_introspect_schema_with_valid_schema_and_headers() {
    ...
    // In a real test environment with NEXUS_API_KEY set, we'd need to provide
    ...
    // Without NEXUS_API_KEY env var set, any key will fail verification
    assert_eq!(response.status(), StatusCode::FORBIDDEN);
}
