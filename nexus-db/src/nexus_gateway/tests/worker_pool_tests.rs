use crate::nexus_engine::traversal_core::NexusGraphEngineOpts;
use crate::nexus_engine::traversal_core::config::Config;
use crate::nexus_engine::{traversal_core::NexusGraphEngine, types::GraphError};
...
async fn test_route_query_not_found() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), NexusError::NotFound { .. }));
}
