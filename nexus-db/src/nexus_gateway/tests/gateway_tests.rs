use crate::nexus_engine::traversal_core::{NexusGraphEngine, NexusGraphEngineOpts};
use crate::nexus_gateway::gateway::{AppState, CoreSetter, GatewayOpts, NexusGateway};
use crate::nexus_gateway::router::router::NexusRouter;
...
fn test_gateway_new_with_cluster_id() {
    unsafe {
        std::env::set_var("NEXUS_CLUSTER_ID", "test-cluster-123");
    }
    let (graph, _temp_dir) = create_test_graph();
    let gateway = NexusGateway::new("127.0.0.1:8080", graph, 8, None, None, None, None);
    ...
    unsafe {
        std::env::remove_var("NEXUS_CLUSTER_ID");
    }
}
...
mod api_key_tests {
    ...
    use crate::protocol::{Format, NexusError};
    use axum::body::Bytes;
    ...
    fn test_verify_key_wrong_key() {
        ...
        if let Err(e) = result {
            assert!(matches!(e, NexusError::InvalidApiKey));
        }
    }
