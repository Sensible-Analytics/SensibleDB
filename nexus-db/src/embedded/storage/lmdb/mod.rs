...
#[cfg(feature = "lmdb")]
use nexus_db::nexus_engine::storage_core::NexusGraphStorage;
...
impl LmdbStorage {
    pub fn new(
        path: &str,
        config: nexus_db::nexus_engine::traversal_core::config::Config,
    ) -> Result<Self> {
        let version_info =
            nexus_db::nexus_engine::storage_core::version_info::VersionInfo::default();
...
impl StorageBackend for LmdbStorage {
    fn get_node(&self, id: u128) -> Result<Option<Node>> {
        ...
        match self.storage.get_node(&txn, &id, &arena) {
            ...
            Err(nexus_db::nexus_engine::types::GraphError::NodeNotFound) => Ok(None),
    ...
    fn get_edge(&self, id: u128) -> Result<Option<Edge>> {
        ...
        match self.storage.get_edge(&txn, &id, &arena) {
            ...
            Err(nexus_db::nexus_engine::types::GraphError::EdgeNotFound) => Ok(None),
            Err(e) => Err(Error::Storage(e.to_string())),
        }
    }
    ...
    fn put_node(&self, _node: Node) -> Result<()> {
        Err(Error::Generic(
            "LMDB storage: put_node requires nexus-db schema".into(),
        ))
    }

    fn put_edge(&self, _edge: Edge) -> Result<()> {
        Err(Error::Generic(
            "LMDB storage: put_edge requires nexus-db schema".into(),
        ))
    }
