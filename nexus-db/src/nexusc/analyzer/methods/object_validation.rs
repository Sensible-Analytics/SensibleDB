...
use crate::nexusc::analyzer::utils::DEFAULT_VAR_NAME;
use crate::nexusc::analyzer::{
    error_codes::ErrorCode, errors::push_query_err, utils::get_field_type_from_item_fields,
};
use crate::nexusc::generator::source_steps::SourceStep;
use crate::nexusc::parser::errors::ParserError;
use crate::{
    generate_error,
    nexusc::{
        analyzer::{
...
fn mark_vector_steps_for_data_fetch(gen_traversal: &mut GeneratedTraversal) {
    use crate::nexusc::generator::traversal_steps::{EdgeType, Step};
    use crate::nexusc::generator::utils::Separator;
...
pub(crate) fn validate_object<'a>(
    ...
    scope: &mut std::collections::HashMap<&'a str, crate::nexusc::analyzer::utils::VariableInfo>,
    gen_query: &mut crate::nexusc::generator::queries::Query,
                ...
                for field_addition in &obj.fields {
                    match &field_addition.value.value {
                        ...
                        FieldValueType::Traversal(tr) => {
                            ...
                            use crate::nexusc::analyzer::methods::traversal_validation::validate_traversal;
                            use crate::nexusc::generator::traversal_steps::NestedTraversalInfo;
                            use crate::nexusc::parser::types::StartNode;
                            ...
                            let mut nested_gen_traversal =
                                crate::nexusc::generator::traversal_steps::Traversal::default();
                            ...
                            let own_closure_param =
                                tr.steps.last().and_then(|step| match &step.step {
                                    crate::nexusc::parser::types::StepType::Closure(cl) => {
                        ...
                        FieldValueType::Expression(expr) => {
                            ...
                            use crate::nexusc::analyzer::methods::traversal_validation::validate_traversal;
                            use crate::nexusc::generator::traversal_steps::{
                                ComputedExpressionInfo, NestedTraversalInfo,
                            };
                            use crate::nexusc::parser::types::ExpressionType;
                            ...
                            if let ExpressionType::MathFunctionCall(_) = &expr.expr {
                                ...
                                if let ExpressionType::Identifier(id) = &expr.expr {
                                    if let Some(var_info) = scope.get(id.as_str()) {
                                        ...
                                        let nested_info = NestedTraversalInfo {
                                            traversal: Box::new(crate::nexusc::generator::traversal_steps::Traversal::default()),
