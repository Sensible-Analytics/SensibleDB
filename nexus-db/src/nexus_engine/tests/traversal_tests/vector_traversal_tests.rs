...
fn setup_test_db() -> (TempDir, Arc<NexusGraphStorage>) {
    ...
    let storage = NexusGraphStorage::new(
        db_path,
        crate::nexus_engine::traversal_core::config::Config::default(),
        Default::default(),
    )
    .unwrap();
...
fn test_drop_vector_removes_edges() {
    ...
    Drop::drop_traversal(
        vectors
            .into_iter()
            .map(Ok::<_, crate::nexus_engine::types::GraphError>),
        storage.as_ref(),
        &mut txn,
    )
    .unwrap();
...
fn test_v_from_type_basic_with_vector_data() {
    ...
    if let crate::nexus_engine::traversal_core::traversal_value::TraversalValue::Vector(v) =
        &results[0]
    {
...
fn test_v_from_type_without_vector_data() {
    ...
    match &results[0] {
        crate::nexus_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) => {
...
fn test_v_from_type_with_properties() {
    ...
    if let crate::nexus_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) = &results[0] {
...
fn test_v_from_type_after_migration() {
    use crate::nexus_engine::storage_core::storage_migration::migrate;
    use crate::protocol::value::Value;
    use std::collections::HashMap;
    ...
    fn clear_metadata(
        storage: &mut crate::nexus_engine::storage_core::NexusGraphStorage,
    ) -> Result<(), crate::nexus_engine::types::GraphError> {
    ...
    for result in &results_without_data {
        if let crate::nexus_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) = result {
