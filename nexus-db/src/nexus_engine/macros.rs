pub mod macros {
    ...
    /// ## Example Use
    /// ```rust
    /// use nexus_db::optional_props;
    /// use nexus_db::protocol::value::Value;
    ///
    ...
    ///
    /// ```rust,ignore
    /// use nexus_db::node_matches;
    /// use nexus_db::protocol::value::Value;
    /// use nexus_db::protocol::items::Node;
    /// use nexus_db::protocol::filterable::Filterable;
    ...
    macro_rules! node_matches {
        ($key:expr, $value:expr) => {
            |node: &nexus_db::protocol::items::Node| {
                if let Some(val) = node.get_property($key) {
                    if let nexus_db::protocol::value::Value::String(val) = &val {
                        Ok(*val == $value)
                    } else {
                        Err(nexus_db::nexus_engine::types::GraphError::from(
    ...
    macro_rules! edge_matches {
        ($key:expr, $value:expr) => {
            |edge: &nexus_db::protocol::items::Edge| {
