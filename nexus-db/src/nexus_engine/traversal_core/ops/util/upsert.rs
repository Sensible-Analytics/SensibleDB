...
fn update_secondary_index(
    bm25: &(
        ...
        crate::nexus_engine::types::SecondaryIndex,
    ),
    ...
    match secondary_index {
        crate::nexus_engine::types::SecondaryIndex::Unique(_) => db
            .put_with_flags(txn, PutFlags::NO_OVERWRITE, &serialized, &id)
            .map_err(|_| GraphError::DuplicateKey(key.to_string()))?,
        crate::nexus_engine::types::SecondaryIndex::Index(_) => db.put(txn, &serialized, &id)?,
        crate::nexus_engine::types::SecondaryIndex::None => unreachable!(),
    }
    Ok(())
}
                ...
                None => {
                    ...
                    for (k, v) in create_props.iter() {
                        ...
                        match secondary_index {
                            ...
                            crate::nexus_engine::types::SecondaryIndex::Index(_) => db
                                .put_with_flags(
                                    self.txn,
                                    PutFlags::APPEND_DUP,
                                    &v_serialized,
                            ...
                            for (k, v) in props.iter() {
                                ...
                                match secondary_index {
                                    crate::nexus_engine::types::SecondaryIndex::Unique(_) => db
                                        ...
                                        .map_err(|_| GraphError::DuplicateKey(k.to_string()))?,
                                    crate::nexus_engine::types::SecondaryIndex::Index(_) => {
                                        db.put(self.txn, &v_serialized, &vector.id)?
                                    }
                                    crate::nexus_engine::types::SecondaryIndex::None => {
                                        unreachable!()
                                    }
                                }
                            }
