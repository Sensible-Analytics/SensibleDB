...
/// ## Example Use
/// ```rust,ignore
/// use sensibledb_db::embed;
/// let db = /* create database */;
/// let query = embed!(db, "Hello, world!");
/// let embedding = embed!(db, "Hello, world!", "text-embedding-ada-002");
/// let embedding = embed!(db, "Hello, world!", "gemini:gemini-embedding-001:SEMANTIC_SIMILARITY");
/// let embedding = embed!(db, "Hello, world!", "text-embedding-ada-002", "http://localhost:8699/embed");
/// ```
macro_rules! embed {
    ($db:expr, $query:expr) => {{
        let embedding_model =
