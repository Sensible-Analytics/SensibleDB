...
impl SensibleGateway {
    ...
    pub fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let all_core_ids = match std::env::var("SENSIBLE_CORES_OVERRIDE") {
            Ok(val) => {
                let override_count: usize = val
                    .parse()
                    .expect("SENSIBLE_CORES_OVERRIDE must be a valid number");
                if override_count > all_core_ids.len() {
                    warn!(
                        "SENSIBLE_CORES_OVERRIDE ({}) exceeds available cores ({}), using all cores",
                        override_count,
                        all_core_ids.len()
                    );
                    all_core_ids
