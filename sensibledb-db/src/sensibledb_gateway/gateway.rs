...
#[cfg(feature = "dev-instance")]
use crate::sensibledb_gateway::builtin::node_by_id::node_details_handler;
#[cfg(feature = "dev-instance")]
use crate::sensibledb_gateway::builtin::node_connections::node_connections_handler;
#[cfg(feature = "dev-instance")]
use crate::sensibledb_gateway::builtin::nodes_by_label::nodes_by_label_handler;
use crate::sensibledb_gateway::introspect_schema::introspect_schema_handler;
...
impl NexusGateway {
    ...
    pub fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        rt.block_on(async move {
            ...
            sensibledb_metrics::init_metrics_system();
            ...
            let shutdown_result = tokio::time::timeout(
                ...
                sensibledb_metrics::shutdown_metrics_system(),
            )
            .await;
...
async fn post_handler(
    ...
    #[cfg(feature = "api-key")]
    {
        use crate::sensibledb_gateway::key_verification::verify_key;
        if let Err(e) = verify_key(req.api_key.as_ref().unwrap()) {
            info!(?e, "Invalid API key");
            sensibledb_metrics::log_event(
                sensibledb_metrics::events::EventType::InvalidApiKey,
                sensibledb_metrics::events::InvalidApiKeyEvent {
    ...
    let input_body = if *sensibledb_metrics::METRICS_ENABLED {
        Some(req.body.clone())
    } else {
        None
    };
    ...
    match res {
        Ok(r) => {
            ...
            if !*sensibledb_metrics::METRICS_ENABLED {
                return r.into_response();
            }
            sensibledb_metrics::log_event(
                sensibledb_metrics::events::EventType::QuerySuccess,
                sensibledb_metrics::events::QuerySuccessEvent {
                    cluster_id: state.cluster_id.clone(),
                    query_name,
                    time_taken_usec: start_time.elapsed().as_micros() as u32,
                },
        ...
        Err(e) => {
            ...
            sensibledb_metrics::log_event(
                sensibledb_metrics::events::EventType::QueryError,
                sensibledb_metrics::events::QueryErrorEvent {
                    cluster_id: state.cluster_id.clone(),
                    query_name,
