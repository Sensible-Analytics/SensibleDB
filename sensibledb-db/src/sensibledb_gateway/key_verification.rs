use crate::protocol::NexusError;
use sha2::{Digest, Sha256};
use std::sync::LazyLock;
use subtle::ConstantTimeEq;

/// API KEY HASH (pre-computed SHA-256 hash read from SENSIBLE_API_KEY env var on startup)
static API_KEY_HASH: LazyLock<[u8; 32]> = LazyLock::new(|| {
    let key = std::env::var("SENSIBLE_API_KEY").unwrap_or_default();
    if key.is_empty() {
        return [0u8; 32];
    }
...
#[inline(always)]
pub(crate) fn verify_key(key: &str) -> Result<(), NexusError> {
    if *API_KEY_HASH == [0u8; 32] {
        return Err(NexusError::InvalidApiKey);
    }
