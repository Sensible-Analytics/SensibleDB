...
use crate::sensibledb_engine::traversal_core::ops::vectors::search::SearchVAdapter;
use crate::sensibledb_engine::vector_core::vector::HVector;
                    ...
                    .collect::<Result<Vec<_>, _>>()
                {
                    Ok(results) => {
                        ...
                        for (i, result) in results.iter().enumerate() {
                            if let crate::sensibledb_engine::traversal_core::traversal_value::TraversalValue::Vector(v) = result {
                                assert!(
                                    v.distance.is_some(),
