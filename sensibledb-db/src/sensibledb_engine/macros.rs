pub mod macros {
    ...
    /// ## Example Use
    /// ```rust
    /// use sensibledb_db::optional_props;
    /// use sensibledb_db::protocol::value::Value;
    ///
    ...
    ///
    /// ```rust,ignore
    /// use sensibledb_db::node_matches;
    /// use sensibledb_db::protocol::value::Value;
    /// use sensibledb_db::protocol::items::Node;
    /// use sensibledb_db::protocol::filterable::Filterable;
    ...
    macro_rules! node_matches {
        ($key:expr, $value:expr) => {
            |node: &sensibledb_db::protocol::items::Node| {
                if let Some(val) = node.get_property($key) {
                    if let sensibledb_db::protocol::value::Value::String(val) = &val {
                        Ok(*val == $value)
                    } else {
                        Err(sensibledb_db::sensibledb_engine::types::GraphError::from(
    ...
    macro_rules! edge_matches {
        ($key:expr, $value:expr) => {
            |edge: &sensibledb_db::protocol::items::Edge| {
