// Copyright 2025 NexusDB Inc.
// SPDX-License-Identifier: AGPL-3.0

//! This is the parser for NexusQL.
...
use crate::sensibledbc::parser::types::{Content, HxFile, Schema, Source};
use location::HasLoc;
...
impl NexusParser {
    pub fn parse_source(input: &Content) -> Result<Source, ParserError> {
        ...
        input.files.iter().try_for_each(|file| {
            ...
            source.source.push('\n');
            let pair = match NexusParser::parse(Rule::source, &file.content) {
                Ok(mut pairs) => pairs
                    .next()
