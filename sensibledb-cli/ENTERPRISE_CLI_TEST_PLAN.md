# SensibleDB CLI Enterprise Cluster Testing Plan
...
This plan covers end-to-end testing for **SensibleDB CLI + enterprise clusters**, focused on:

1. Pushing new `sensibledb_dsl` Rust queries via CLI after generating `queries.json`
2. Running `sensibledb sync` for enterprise clusters
...
- CLI: `~/GitHub/sensibledb/sensibledb-cli`
- Dashboard backend/API: `~/GitHub/sensibledb-cloud-build/build-gateway`
- Enterprise runtime/provisioner: `~/GitHub/sensibledb-hyperscale`
- Enterprise query DSL generator: `~/GitHub/sensibledb-enterprise-ql`
...
- [ ] Confirm test user can run `sensibledb auth login`
...
- [x] Prepare valid enterprise query project (Cargo.toml + `src/*.rs` + `sensibledb_dsl::generate()` in main)
...
- [ ] `sensibledb add <name> --cloud` in enterprise workspace allows enterprise cluster creation path
- [ ] `sensibledb add <name> --cloud` in non-enterprise workspace forces standard cluster path
...
## Phase 2 - Enterprise Push Path (`sensibledb push`)
...
- [x] Push preserves optional `sensibledb.toml` upload behavior
...
## Phase 3 - Enterprise Sync Path (`sensibledb sync`)
...
- [x] `sensibledb sync <enterprise-instance>` successfully refreshes `sensibledb.toml` project/cluster metadata
...
- [ ] Add tests for enterprise deploy-by-cluster-id behavior with stale sensibledb.toml mapping

### `sensibledb-cloud-build/build-gateway`

- [ ] Add handler tests for CLI enterprise deploy endpoint (base64, empty payload, unsafe paths)
- [ ] Add handler tests for CLI enterprise sync endpoint (authz, metadata shape, sensibledb.toml retrieval)
...
- [x] Run `sensibledb push <enterprise-instance>`
...
- [x] Run `sensibledb sync <enterprise-instance>` and verify local files
