...
# SensibleDB CLI Installer
# Cross-platform installer for SensibleDB CLI

readonly REPO="Sensible-Analytics/SensibleDB"
readonly BINARY_NAME="sensibledb"
...
set_install_dir() {
    if [[ -n "$INSTALL_DIR" ]]; then
        ...
        if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
            INSTALL_DIR="/c/Program Files/sensibledb"
        else
...
setup_path() {
    ...
    if [[ -f "$shell_config" ]] && ! grep -Fq "$path_line" "$shell_config"; then
        echo "" >> "$shell_config"
        echo "# Added by SensibleDB CLI installer" >> "$shell_config"
...
check_docker_desktop() {
    ...
    log_info "SensibleDB CLI requires Docker to be running for local development"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if brew install --cask docker; then
            ...
            log_info "You'll need to complete the Docker Desktop setup before using SensibleDB CLI"
        else
        ...
        log_warn "IMPORTANT: You need to have the Docker daemon running to use SensibleDB CLI"
...
main() {
    log_info "SensibleDB CLI Installer"
    ...
    log_info "2. Run: sensibledb --version"
    log_info "3. Run: sensibledb --help"
    log_info ""
    log_info "To update in the future, run: sensibledb update"
    log_info ""
    log_info "Anonymous metrics are enabled by default."
    log_info "To help us improve SensibleDB, please consider enabling full metrics."
    log_info "Run: sensibledb metrics --full"
    log_info ""
    log_info "To disable metrics, run: sensibledb metrics --off"
    log_info ""
    log_info "To show metrics status, run: sensibledb metrics status"
}
