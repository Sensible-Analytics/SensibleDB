...
readonly REPO="SensibleDB/sensibledb-db"
readonly BINARY_NAME="nexus"
...
set_install_dir() {
    if [[ -n "$INSTALL_DIR" ]]; then
        ...
        if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
            INSTALL_DIR="/c/Program Files/nexus"
        else
...
main() {
    ...
    log_info "2. Run: nexus --version"
    log_info "3. Run: nexus --help"
    log_info ""
    log_info "To update in the future, run: nexus update"
    log_info ""
    log_info "Anonymous metrics are enabled by default."
    log_info "To help us improve Nexus, please consider enabling full metrics."
    log_info "Run: nexus metrics --full"
    log_info ""
    log_info "To disable metrics, run: nexus metrics --off"
    log_info ""
    log_info "To show metrics status, run: nexus metrics status"
}
