...
fn extract_standard_snapshot_config(
    ...
    let mut nexus_configs = remote_config
        .cloud
        .values()
    ...
    let first = nexus_configs.next()?;
    if nexus_configs.next().is_none() {
        Some(first)
    } else {
        None
    }
...
async fn fetch_standard_cluster_snapshot_config(
    ...
    Ok(snapshot_config_from_remote_toml(
        sync_response.nexus_toml.as_deref(),
        source,
    ))
}
...
fn update_project_queries_path_in_nexus_toml(
    project_root: &Path,
    queries_path: &Path,
) -> Result<()> {
    let nexus_toml_path = project_root.join("sensibledb.toml");
    let mut config = NexusConfig::from_file(&nexus_toml_path)
...
async fn reconcile_project_config_from_cloud(
    ...
    let nexus_toml_path = project_root.join("sensibledb.toml");
    let mut config = if nexus_toml_path.exists() {
        NexusConfig::from_file(&nexus_toml_path)
...
async fn run_project_sync_flow(project: &ProjectContext, assume_yes: bool) -> Result<()> {
    ...
    if is_enterprise {
        ...
        {
            update_project_queries_path_in_nexus_toml(&project.root, &selected_queries_relative)?;
            Step::verbose_substep(&format!(
...
async fn sync_enterprise_from_cluster_id(api_key: &str, cluster_id: &str) -> Result<()> {
    ...
    let cwd = std::env::current_dir()?;
    let remote_config = sync_response.nexus_toml.as_deref().and_then(|remote_toml| {
...
async fn pull_from_local_instance(project: &ProjectContext, instance_name: &str) -> Result<()> {
    ...
    let container_dir = workspace.join("sensibledb-container");
    ...
    match instance_config {
        InstanceInfo::Enterprise(config) => {
            ...
            if let SyncReconciliationOutcome::Pulled = sync_outcome
                && project.config.project.queries != selected_queries_relative
            {
                update_project_queries_path_in_nexus_toml(
                    &project.root,
                    &selected_queries_relative,
                )?;
                Step::verbose_substep(&format!(
