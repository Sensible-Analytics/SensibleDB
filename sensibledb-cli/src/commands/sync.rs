...
use crate::commands::integrations::nexus::NexusManager;
use crate::commands::integrations::nexus::cloud_base_url;
use crate::config::{
    AvailabilityMode, BuildMode, CloudConfig, CloudInstanceConfig, DbConfig,
    EnterpriseInstanceConfig, NexusConfig, InstanceInfo, WorkspaceConfig,
};
use crate::output::{Operation, Step};
use crate::project::ProjectContext;
use crate::prompts;
use crate::utils::sensibledbc_utils::{
...
async fn fetch_enterprise_sync_response_with_remote_empty_fallback(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| eyre!("Failed to connect to SensibleDB Cloud: {}", e))?;
    ...
    match response.status() {
        ...
        reqwest::StatusCode::UNAUTHORIZED => Err(eyre!(
            "Authentication failed. Run 'nexus auth login' to re-authenticate."
        )),
...
fn parse_and_sanitize_remote_config(
    remote_toml: &str,
    source: &str,
) -> Option<crate::config::NexusConfig> {
    let mut remote_config = match toml::from_str::<crate::config::NexusConfig>(remote_toml) {
        Ok(config) => config,
        Err(e) => {
            print_warning(&format!(
                "Ignoring remote sensibledb.toml from {}: failed to parse ({})",
                source, e
            ));
            return None;
        }
...
fn resolve_remote_queries_dir(
    base_dir: &Path,
    remote_config: Option<&crate::config::NexusConfig>,
) -> PathBuf {
...
async fn push_local_snapshot_to_cluster(
    ...
    let nexus = NexusManager::new(&refreshed_project);
...
fn extract_standard_snapshot_config(
    remote_config: &NexusConfig,
    cluster_id: &str,
) -> Option<CloudInstanceConfig> {
    if let Some(config) = remote_config.cloud.values().find_map(|entry| match entry {
        CloudConfig::Nexus(config) if config.cluster_id == cluster_id => Some(config.clone()),
        _ => None,
    }) {
        return Some(config);
    }

    let mut sensibledb_configs = remote_config
        .cloud
        .values()
        .filter_map(|entry| match entry {
            CloudConfig::Nexus(config) => Some(config.clone()),
            _ => None,
        });

    let first = sensibledb_configs.next()?;
    if sensibledb_configs.next().is_none() {
        Some(first)
    } else {
        None
    }
...
) -> Result<HashMap<String, NexusConfig>> {
...
fn merged_standard_cluster_config(
    ...
    snapshot_config: Option<&NexusConfig>,
) -> CloudInstanceConfig {
...
fn selected_project_queries_path(selected_snapshot: Option<&NexusConfig>) -> Option<PathBuf> {
    selected_snapshot.map(|snapshot| snapshot.project.queries.clone())
}

fn resolve_selected_project_queries_path(selected_snapshot: Option<&NexusConfig>) -> PathBuf {
    selected_project_queries_path(selected_snapshot)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_QUERIES_DIR))
}

fn update_project_queries_path_in_sensibledb_toml(
    project_root: &Path,
    queries_path: &Path,
) -> Result<()> {
    let sensibledb_toml_path = project_root.join("sensibledb.toml");
    let mut config = NexusConfig::from_file(&sensibledb_toml_path)
        .map_err(|e| eyre!("Failed to load sensibledb.toml for queries path update: {}", e))?;

    config.project.queries = sanitize_relative_path(queries_path)?;
    config
        .save_to_file(&sensibledb_toml_path)
        .map_err(|e| eyre!("Failed to update queries path in sensibledb.toml: {}", e))?;
...
async fn reconcile_project_config_from_cloud(
    ...
    let sensibledb_toml_path = project_root.join("sensibledb.toml");
    let mut config = if sensibledb_toml_path.exists() {
        NexusConfig::from_file(&sensibledb_toml_path)
            .map_err(|e| eyre!("Failed to load sensibledb.toml: {}", e))?
    } else {
        NexusConfig {
    ...
    let existing_standard_configs: HashMap<String, CloudInstanceConfig> = config
        ...
        .values()
        .filter_map(|entry| match entry {
            CloudConfig::Nexus(instance) => Some((instance.cluster_id.clone(), instance.clone())),
            _ => None,
        })
        .collect();
    ...
    // Remove only Nexus-managed cloud entries; preserve FlyIo, Ecr
    config
        .cloud
        .retain(|_name, entry| !matches!(entry, CloudConfig::Nexus(_)));
    ...
    for cluster in &project_clusters.standard {
        ...
        let inserted_name = insert_unique_cloud_instance_name(
            ...
            &cluster.cluster_id,
            CloudConfig::Nexus(instance_config),
        );
    ...
    for cluster in &project_clusters.enterprise {
        ...
        {
            print_warning(&format!(
                "Enterprise cluster '{}' uses different gateway ({}) and DB ({}) counts; sensibledb.toml stores these as min_instances/max_instances for compatibility.",
    ...
    config
        .save_to_file(&sensibledb_toml_path)
        .map_err(|e| eyre!("Failed to write sensibledb.toml: {}", e))?;
...
async fn run_project_sync_flow(project: &ProjectContext, assume_yes: bool) -> Result<()> {
    prompts::intro(
        "nexus sync",
        Some(&format!(
            "Using project '{}' from sensibledb.toml. Select a cluster to sync from.",
    ...
    if is_enterprise {
        ...
        {
            update_project_queries_path_in_sensibledb_toml(&project.root, &selected_queries_relative)?;
            Step::verbose_substep(&format!(
    ...
    .await?;
    crate::output::info(
        "Updated sensibledb.toml with canonical project and cluster metadata from SensibleDB Cloud.",
    );
...
pub async fn run(instance_name: Option<String>, assume_yes: bool) -> Result<()> {
    ...
    if let Some(instance_name) = instance_name {
        let project = project.ok_or_else(|| {
            eyre!("No sensibledb.toml found. Run 'nexus init' to create a project first.")
        })?;
    ...
    if !prompts::is_interactive() {
        return Err(eyre!(
            "No instance specified. Run 'nexus sync <instance>' or run interactively in a project directory."
        ));
    }
...
async fn run_workspace_sync_flow() -> Result<()> {
    prompts::intro(
        "nexus sync",
        Some("No sensibledb.toml found. Select a workspace and cluster to sync from."),
    )?;
    ...
    if workspace_clusters.standard.is_empty() && workspace_clusters.enterprise.is_empty() {
        return Err(eyre!(
            "No clusters found in this workspace. Deploy a cluster first with 'nexus push'."
        ));
    }
    ...
    if is_enterprise {
        // Enterprise sync
        sync_enterprise_from_cluster_id(&credentials.nexus_admin_key, &cluster_id).await
    } else {
        // Standard sync
        sync_from_cluster_id(&credentials.nexus_admin_key, &cluster_id).await
    }
}
...
async fn sync_from_cluster_id(api_key: &str, cluster_id: &str) -> Result<()> {
    ...
    .await?;
    files_written += 1;
    Step::verbose_substep("  Wrote sensibledb.toml (canonical cloud metadata)");
...
async fn sync_enterprise_from_cluster_id(api_key: &str, cluster_id: &str) -> Result<()> {
    ...
    let cwd = std::env::current_dir()?;
    let remote_config = sync_response.sensibledb_toml.as_deref().and_then(|remote_toml| {
...
async fn pull_from_local_instance(project: &ProjectContext, instance_name: &str) -> Result<()> {
    ...
    let container_dir = workspace.join("nexus-container");

    if !container_dir.exists() {
        op.failure();
        return Err(eyre!(
            "Instance '{instance_name}' has not been built yet. Run 'nexus build {instance_name}' first."
        ));
    }
    ...
    match instance_config {
        InstanceInfo::Enterprise(config) => {
            ...
            if let SyncReconciliationOutcome::Pulled = sync_outcome
                && project.config.project.queries != selected_queries_relative
            {
                update_project_queries_path_in_sensibledb_toml(
                    &project.root,
                    &selected_queries_relative,
                )?;
                Step::verbose_substep(&format!(
        ...
        InstanceInfo::FlyIo(_) => Err(eyre!(
            "Sync is only supported for SensibleDB Cloud instances, not Fly.io deployments"
        )),
        InstanceInfo::Ecr(_) => Err(eyre!(
            "Sync is only supported for SensibleDB Cloud instances, not ECR deployments"
        )),
...
mod tests {
    ...
    fn sample_snapshot(instance_name: &str, config: CloudInstanceConfig) -> NexusConfig {
        let mut cloud = HashMap::new();
        cloud.insert(instance_name.to_string(), CloudConfig::Nexus(config));
        NexusConfig {
            project: crate::config::ProjectConfig {
    ...
    fn insert_unique_cloud_instance_name_adds_suffix_on_collision() {
        ...
        cloud.insert(
            "prod".to_string(),
            CloudConfig::Nexus(cloud_db_config(20, "FIRST")),
        );
        ...
        let inserted_name = insert_unique_cloud_instance_name(
            ...
            "prod",
            "cluster-abc12345",
            CloudConfig::Nexus(cloud_db_config(30, "SECOND")),
        );
