...
async fn check_instance(
    ...
    // Step 2: Ensure nexus repo is cached (reuse from build.rs)
    let mut repo_step = Step::with_messages("Syncing repository", "Repository synced");
    repo_step.start();
    build::ensure_nexus_repo_cached().await?;
    repo_step.done();
    ...
    // Step 5: Copy generated files to nexus-repo-copy for cargo check
    let instance_workspace = project.instance_workspace(instance_name);
    let generated_src = instance_workspace.join("sensibledb-container/src");
    let cargo_check_src = instance_workspace.join("nexus-repo-copy/sensibledb-container/src");
    ...
    let sensibledb_container_dir = instance_workspace.join("nexus-repo-copy/sensibledb-container");
    let cargo_output = run_cargo_check(&sensibledb_container_dir)?;
...
async fn check_all_instances(
    ...
    if instances.is_empty() {
        return Err(eyre::eyre!(
            "No instances found in sensibledb.toml. Add at least one instance to check."
        ));
    }
...
fn validate_project_syntax(project: &ProjectContext) -> Result<()> {
    ...
    // Generate content and validate using sensibledb-db parsing logic
...
fn run_cargo_check(sensibledb_container_dir: &Path) -> Result<CargoCheckOutput> {
    let output = Command::new("cargo")
        .arg("check")
        .arg("--color=never") // Disable color codes for cleaner output
        .current_dir(sensibledb_container_dir)
        .output()
