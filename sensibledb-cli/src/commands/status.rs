...
pub async fn run() -> Result<()> {
    ...
    let project = match ProjectContext::find_and_load(None) {
        Ok(project) => project,
        Err(_) => {
            print_error("Not in a Nexus project directory. Run 'nexus init' to create one.");
            return Ok(());
        }
    };

    print_header("Nexus Project Status");
    ...
    let mut sensibledb_cloud_instances = Vec::new();
    let mut flyio_instances = Vec::new();
    let mut ecr_instances = Vec::new();

    for (name, config) in &project.config.cloud {
        match config {
            crate::config::CloudConfig::Nexus(sensibledb_config) => {
                sensibledb_cloud_instances.push((name, &sensibledb_config.cluster_id));
            }
    ...
    for (name, cluster_id) in sensibledb_cloud_instances {
        print_field(
            &format!("{name} (SensibleDB Cloud)"),
