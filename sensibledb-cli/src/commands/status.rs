...
pub async fn run() -> Result<()> {
    ...
    let mut nexus_cloud_instances = Vec::new();
    let mut flyio_instances = Vec::new();
    let mut ecr_instances = Vec::new();

    for (name, config) in &project.config.cloud {
        match config {
            crate::config::CloudConfig::Nexus(nexus_config) => {
                nexus_cloud_instances.push((name, &nexus_config.cluster_id));
            }
    ...
    for (name, cluster_id) in nexus_cloud_instances {
        print_field(
