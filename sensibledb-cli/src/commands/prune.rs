...
pub async fn run(instance: Option<String>, all: bool) -> Result<()> {
    ...
    match ProjectContext::find_and_load(None) {
        ...
        Err(_) => {
            // Outside a Nexus project - offer system-wide clean
            if instance.is_some() || all {
                return Err(project_error("not in a Nexus project directory")
                    .with_hint("use 'nexus prune' without arguments for system-wide cleanup")
                    .into());
            }
...
async fn prune_unused_resources(project: &ProjectContext) -> Result<()> {
    ...
    print_lines(&[
        ...
        "",
        "Hint: To clean all instances while preserving volumes, use 'nexus prune --all'",
        "      To clean a specific instance, use 'nexus prune <instance_name>'",
    ]);
    print_newline();
...
async fn prune_system_wide() -> Result<()> {
    print_warning("You are not in a Nexus project directory.");
    print_lines(&[
        "This will remove ALL Nexus-related Docker images from your system.",
    ...
    for runtime in [ContainerRuntime::Docker, ContainerRuntime::Podman] {
        if DockerManager::check_runtime_available(runtime).is_ok() {
            ...
            DockerManager::clean_all_sensibledb_images(runtime)?;
