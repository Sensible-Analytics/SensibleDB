...
use crate::project::{ProjectContext, get_sensibledb_repo_cache};
use crate::prompts;
use crate::utils::{
    copy_dir_recursive_excluding, diagnostic_source,
    sensibledbc_utils::{collect_hx_contents, collect_hx_files},
...
use sensibledb_db::{
    sensibledb_engine::traversal_core::config::Config,
    sensibledbc::{
        analyzer::analyze,
...
const NEXUS_REPO_URL: &str = "https://github.com/nexusdb/nexus-db.git";
    ...
    let instance_name = match instance_name {
        ...
        None if prompts::is_interactive() => {
            ...
            prompts::intro(
                "nexus build",
                Some(
                    "This will build your selected instance based on the configuration in sensibledb.toml.",
                ),
            )?;
...
pub async fn run_build_steps(
    ...
    repo_step.start();
    ensure_sensibledb_repo_cached().await?;
    repo_step.done();
...
pub(crate) async fn ensure_sensibledb_repo_cached() -> Result<()> {
    let _lock = repo_cache_lock().lock().await;
    let repo_cache = get_sensibledb_repo_cache()?;

    if needs_cache_recreation(&repo_cache)? {
        recreate_sensibledb_cache(&repo_cache).await?;
    } else if repo_cache.exists() {
        update_sensibledb_cache(&repo_cache).await?;
    } else {
        create_sensibledb_cache(&repo_cache).await?;
    }
...
async fn recreate_sensibledb_cache(repo_cache: &std::path::Path) -> Result<()> {
    std::fs::remove_dir_all(repo_cache)?;
    create_sensibledb_cache(repo_cache).await
}

async fn create_sensibledb_cache(repo_cache: &std::path::Path) -> Result<()> {
    Step::verbose_substep("Caching SensibleDB repository (first time setup)...");
...
async fn update_sensibledb_cache(repo_cache: &std::path::Path) -> Result<()> {
    Step::verbose_substep("Updating SensibleDB repository cache...");
...
fn create_git_cache(repo_cache: &std::path::Path) -> Result<()> {
    let output = std::process::Command::new("git")
        .args(["clone", NEXUS_REPO_URL, &repo_cache.to_string_lossy()])
        .output()?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        let error = crate::errors::CliError::new("failed to clone SensibleDB repository")
...
fn update_git_cache(repo_cache: &std::path::Path) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(eyre::eyre!(
            "Failed to update SensibleDB repository:\n{}",
            stderr
        ));
    }
...
pub(crate) async fn prepare_instance_workspace(
    ...
    let repo_copy_path = instance_workspace.join("nexus-repo-copy");
...
pub(crate) async fn compile_project(
    ...
    // Create nexus-container directory in instance workspace for generated files
    let instance_workspace = project.instance_workspace(instance_name);
    let sensibledb_container_dir = instance_workspace.join("nexus-container");
    let src_dir = sensibledb_container_dir.join("src");

    // Create the directories
    fs::create_dir_all(&src_dir)?;

    // Generate config.hx.json from sensibledb.toml
    ...
    Step::verbose_substep("Generating Rust code from Nexus queries...");

    // Collect all .hx files for compilation
    let hx_files = collect_hx_files(&project.root, &project.config.project.queries)?;

    // Generate content and compile using nexus-db compilation logic
    let (analyzed_source, metrics_data) = compile_sensibledb_files(&hx_files, &src_dir)?;
...
/// Uses the nexus parser to parse the content into a Source object
fn parse_content(content: &Content) -> Result<Source> {
    let source = NexusParser::parse_source(content).map_err(|e| eyre::eyre!("Parse error: {e}"))?;
    Ok(source)
}
...
fn handle_docker_rust_compilation_failure(
    ...
    println!();
    println!("This may indicate a bug in the Nexus code generator.");
    println!();

    // Offer to create GitHub issue
    print_warning("You can report this issue to help improve Nexus.");
    println!();
...
fn build_binary_using_cargo(
    ...
    // <path-to-.sensibledb>/<instance_name>/sensibledb-repo-copy/sensibledb-container/
    let current_dir = project
        .sensibledb_dir
        .join(instance_name)
        .join("sensibledb-repo-copy")
