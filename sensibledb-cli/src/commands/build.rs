...
use crate::project::{ProjectContext, get_nexus_repo_cache};
use crate::prompts;
use crate::utils::{
...
const SENSIBLE_REPO_URL: &str = "https://github.com/sensibledb/sensibledb-db.git";
...
pub async fn run_build_steps(
    ...
    repo_step.start();
    ensure_nexus_repo_cached().await?;
    repo_step.done();
...
pub(crate) async fn ensure_nexus_repo_cached() -> Result<()> {
    let _lock = repo_cache_lock().lock().await;
    let repo_cache = get_nexus_repo_cache()?;

    if needs_cache_recreation(&repo_cache)? {
        recreate_nexus_cache(&repo_cache).await?;
    } else if repo_cache.exists() {
        update_nexus_cache(&repo_cache).await?;
    } else {
        create_nexus_cache(&repo_cache).await?;
    }
...
async fn recreate_nexus_cache(repo_cache: &std::path::Path) -> Result<()> {
    std::fs::remove_dir_all(repo_cache)?;
    create_nexus_cache(repo_cache).await
}

async fn create_nexus_cache(repo_cache: &std::path::Path) -> Result<()> {
...
async fn update_nexus_cache(repo_cache: &std::path::Path) -> Result<()> {
...
fn create_dev_cache(repo_cache: &std::path::Path) -> Result<()> {
    let workspace_root = std::path::Path::new(CARGO_MANIFEST_DIR)
        .parent() // sensibledb-cli -> sensibledb-db
...
fn create_git_cache(repo_cache: &std::path::Path) -> Result<()> {
    let output = std::process::Command::new("git")
        .args(["clone", SENSIBLE_REPO_URL, &repo_cache.to_string_lossy()])
        .output()?;
...
pub(crate) async fn compile_project(
    ...
    // Create sensibledb-container directory in instance workspace for generated files
    let instance_workspace = project.instance_workspace(instance_name);
    let sensibledb_container_dir = instance_workspace.join("sensibledb-container");
    ...
    // Generate content and compile using sensibledb-db compilation logic
    let (analyzed_source, metrics_data) = compile_nexus_files(&hx_files, &src_dir)?;
...
fn build_binary_using_cargo(
    ...
    // <path-to-.sensibledb>/<instance_name>/nexus-repo-copy/sensibledb-container/
    let current_dir = project
        .nexus_dir
        .join(instance_name)
        .join("nexus-repo-copy")
