use crate::{
    AuthAction,
    commands::integrations::nexus::cloud_base_url,
...
async fn login() -> Result<()> {
    output::info("Logging into SensibleDB Cloud");

    let home = dirs::home_dir().ok_or_eyre("Cannot find home directory")?;
    let config_path = home.join(".sensibledb");
    ...
    output::info("Your credentials are stored in ~/.sensibledb/credentials");

    Ok(())
}

async fn logout() -> Result<()> {
    output::info("Logging out of SensibleDB Cloud");
...
async fn create_key(cluster: &str) -> Result<()> {
    ...
    output::info("Update SENSIBLE_API_KEY in your environment before running queries.");
...
pub struct Credentials {
    ...
    pub(crate) nexus_admin_key: String,
}

impl Credentials {
    pub fn is_authenticated(&self) -> bool {
        !self.user_id.is_empty() && !self.nexus_admin_key.is_empty()
    }
    ...
    pub(crate) fn write_to_file(&self, path: &PathBuf) {
        let content = format!(
            "nexus_user_id={}\nnexus_user_key={}",
            self.user_id, self.nexus_admin_key
        );
    ...
    fn parse_key_value_format(content: &str) -> Result<Self> {
        ...
        for line in content.lines() {
            ...
            if let Some((key, value)) = line.split_once('=') {
                match key.trim() {
                    "nexus_user_id" => user_id = Some(value.trim().to_string()),
                    "nexus_user_key" => nexus_admin_key = Some(value.trim().to_string()),
        ...
        Ok(Credentials {
            user_id: user_id.ok_or_eyre("Missing nexus_user_id in credentials file")?,
            nexus_admin_key: nexus_admin_key
                .ok_or_eyre("Missing nexus_user_key in credentials file")?,
        })
    }
}

/// Check that the user is authenticated with SensibleDB Cloud.
...
pub async fn require_auth() -> Result<Credentials> {
    ...
    output::warning("Not authenticated with SensibleDB Cloud");

    if !crate::prompts::is_interactive() {
        return Err(eyre!("Run 'nexus auth login' first."));
    }
    ...
    if !should_login {
        return Err(eyre!(
            "Authentication required. Run 'nexus auth login' to authenticate."
        ));
    }
