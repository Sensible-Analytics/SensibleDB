//! Dashboard management for Nexus projects
...
const DASHBOARD_IMAGE: &str = "public.ecr.aws/p8l2s5f1/nexus-dashboard";
const DASHBOARD_TAG: &str = "latest";
const DASHBOARD_CONTAINER_NAME: &str = "nexus-dashboard";
const DEFAULT_SENSIBLE_PORT: u16 = 6969;
...
pub async fn run(action: DashboardAction) -> Result<()> {
    match action {
        DashboardAction::Start {
            ...
            attach,
            restart,
        } => start(instance, port, host, nexus_port, attach, restart).await,
...
async fn start(
    ...
    if is_dashboard_running(runtime)? {
        if restart {
            ...
            output::info("Use 'nexus dashboard stop' to stop it, or '--restart' to restart");
            return Ok(());
        }
    }

    // Warn if --nexus-port is specified without --host
    if host.is_none() && nexus_port != DEFAULT_SENSIBLE_PORT {
        output::warning("--nexus-port is ignored without --host; using project config or defaults");
    }
    ...
    let (env_vars, display_info) = if let Some(host) = host {
        ...
        prepare_direct_env_vars(&host, nexus_port, runtime)?
    } else {
    ...
    if !attach {
        ...
        let mut details: Vec<(&str, String)> = vec![
            ("URL", url.clone()),
            ("Nexus Host", display_info.host.clone()),
            ("Nexus Port", display_info.nexus_port.to_string()),
        ];
        ...
        println!();
        output::info("Run 'nexus dashboard stop' to stop the dashboard");
...
fn prepare_direct_env_vars(
    ...
    let env_vars = vec![
        format!("SENSIBLE_HOST={docker_host}"),
        format!("SENSIBLE_PORT={nexus_port}"),
    ];
...
async fn prepare_env_vars_from_context(
    ...
    match ProjectContext::find_and_load(None) {
        Ok(project) => {
            ...
            let (host, nexus_port, mode) = if instance_config.is_local() {
                let port = instance_config.port().unwrap_or(DEFAULT_SENSIBLE_PORT);
        ...
        Err(_) => {
            ...
            output::info(&format!(
                "No sensibledb.toml found, using default connection (localhost:{DEFAULT_SENSIBLE_PORT})"
            ));
            prepare_direct_env_vars("localhost", DEFAULT_SENSIBLE_PORT, runtime)
        }
    }
}
    ...
    match instance {
        ...
        None => {
            ...
            if instances.is_empty() {
                return Err(eyre!("No instances configured in sensibledb.toml"));
            }
...
async fn check_dev_mode_requirement(
    ...
    if !prompts::is_interactive() {
        return Err(eyre!(
            ...
            Redeploy with 'nexus push {} --dev' or update build_mode to 'dev' in sensibledb.toml.",
            instance_name,
            instance_name
        ));
    }
    ...
    if !should_redeploy {
        return Err(eyre!(
            "Dashboard requires dev mode. Update build_mode to 'dev' in sensibledb.toml or use --host flag to connect directly."
        ));
    }
    ...
    if instance_config.is_local() {
        ...
        let config_path = project.root.join("sensibledb.toml");
        ...
        output::info("Using a one-time dev override for this redeploy; sensibledb.toml is unchanged.");
...
async fn check_instance_running(project: &ProjectContext, instance_name: &str) -> Result<()> {
    ...
    let container_prefix = format!("nexus-{}-{}", project.config.project.name, instance_name);
    ...
    if !prompts::is_interactive() {
        return Err(eyre!(
            "Instance '{}' is not running. Start it with 'nexus push {}'",
            instance_name,
            instance_name
        ));
    }
    ...
    if !should_push {
        return Err(eyre!(
            "Dashboard requires a running instance. Build and start it with 'nexus push {}'",
            instance_name
        ));
    }
...
async fn prepare_environment_vars(
    ...
    if instance_config.is_local() {
        ...
        env_vars.push(format!("SENSIBLE_HOST={host}"));
        env_vars.push(format!("SENSIBLE_PORT={port}"));
        env_vars.push(format!("SENSIBLE_INSTANCE={instance_name}"));
    } else {
        ...
        env_vars.push(format!("SENSIBLE_CLOUD_URL={cloud_url}"));
        env_vars.push(format!("SENSIBLE_API_KEY={}", credentials.nexus_admin_key));
        env_vars.push(format!("SENSIBLE_USER_ID={}", credentials.user_id));
        env_vars.push(format!("SENSIBLE_INSTANCE={instance_name}"));

        // Add cluster ID for SensibleDB Cloud instances
        if let Some(cluster_id) = instance_config.cluster_id() {
            env_vars.push(format!("SENSIBLE_CLUSTER_ID={cluster_id}"));
        }
    }
...
fn get_cloud_url(instance_config: &InstanceInfo) -> Result<String> {
    match instance_config {
        InstanceInfo::Nexus(config) => Ok(format!(
            "{}/clusters/{}",
            cloud_base_url(),
            config.cluster_id
        )),
...
fn status() -> Result<()> {
    ...
    if let Ok(cmd_output) = cmd_output {
        ...
        for line in env_output.lines() {
            if let Some(instance) = line.strip_prefix("SENSIBLE_INSTANCE=") {
                println!("  {}: {instance}", "Instance".bright_white().bold());
            }
            if let Some(host) = line.strip_prefix("SENSIBLE_HOST=") {
                println!("  {}: {host}", "Nexus Host".bright_white().bold());
            }
            if let Some(port) = line.strip_prefix("SENSIBLE_PORT=") {
                println!("  {}: {port}", "Nexus Port".bright_white().bold());
            }
            if line.starts_with("SENSIBLE_CLOUD_URL=") {
