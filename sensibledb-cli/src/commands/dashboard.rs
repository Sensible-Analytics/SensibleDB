...
const DEFAULT_NEXUS_PORT: u16 = 6969;
...
async fn start(
    ...
    if host.is_none() && nexus_port != DEFAULT_NEXUS_PORT {
...
fn prepare_direct_env_vars(
    ...
    let env_vars = vec![
        format!("NEXUS_HOST={docker_host}"),
        format!("NEXUS_PORT={nexus_port}"),
    ];
    ...
    match ProjectContext::find_and_load(None) {
        Ok(project) => {
            ...
            let (host, nexus_port, mode) = if instance_config.is_local() {
                let port = instance_config.port().unwrap_or(DEFAULT_NEXUS_PORT);
        ...
        Err(_) => {
            ...
            output::info(&format!(
                "No sensibledb.toml found, using default connection (localhost:{DEFAULT_NEXUS_PORT})"
            ));
            prepare_direct_env_vars("localhost", DEFAULT_NEXUS_PORT, runtime)
        }
    }
}
...
async fn check_dev_mode_requirement(
    ...
    if !prompts::is_interactive() {
        return Err(eyre!(
            ...
            Redeploy with 'sensibledb push {} --dev' or update build_mode to 'dev' in sensibledb.toml.",
            instance_name,
            instance_name
        ));
    }
...
async fn prepare_environment_vars(
    ...
    if instance_config.is_local() {
        ...
        env_vars.push(format!("NEXUS_HOST={host}"));
        env_vars.push(format!("NEXUS_PORT={port}"));
        env_vars.push(format!("NEXUS_INSTANCE={instance_name}"));
    } else {
        ...
        env_vars.push(format!("NEXUS_CLOUD_URL={cloud_url}"));
        env_vars.push(format!("NEXUS_API_KEY={}", credentials.nexus_admin_key));
        env_vars.push(format!("NEXUS_USER_ID={}", credentials.user_id));
        env_vars.push(format!("NEXUS_INSTANCE={instance_name}"));

        // Add cluster ID for SensibleDB Cloud instances
        if let Some(cluster_id) = instance_config.cluster_id() {
            env_vars.push(format!("NEXUS_CLUSTER_ID={cluster_id}"));
        }
    }
...
fn status() -> Result<()> {
    ...
    if let Ok(cmd_output) = cmd_output {
        ...
        for line in env_output.lines() {
            if let Some(instance) = line.strip_prefix("NEXUS_INSTANCE=") {
                println!("  {}: {instance}", "Instance".bright_white().bold());
            }
            if let Some(host) = line.strip_prefix("NEXUS_HOST=") {
                println!("  {}: {host}", "Nexus Host".bright_white().bold());
            }
            if let Some(port) = line.strip_prefix("NEXUS_PORT=") {
                println!("  {}: {port}", "Nexus Port".bright_white().bold());
            }
            if line.starts_with("NEXUS_CLOUD_URL=") {
