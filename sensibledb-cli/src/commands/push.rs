...
pub async fn run(
    ...
    let instance_name = match instance_name {
        ...
        None if prompts::is_interactive() => {
            ...
            prompts::intro(
                "nexus push",
                Some(
                    "This will build and redeploy your selected instance based on the configuration in sensibledb.toml.",
                ),
            )?;
    ...
    // Check auth early for SensibleDB Cloud / Enterprise instances
    if matches!(
        &instance_config,
        InstanceInfo::Nexus(_) | InstanceInfo::Enterprise(_)
    ) {
        require_auth().await?;
    }
...
async fn push_local_instance(
    ...
    if Verbosity::current().show_normal() {
        Operation::print_details(&[
            ...
            (
                "Container",
                &format!("nexus-{project_name}-{instance_name}"),
            ),
            (
                "Data volume",
...
async fn push_cloud_instance(
    ...
    if let InstanceInfo::Enterprise(config) = &instance_config {
        let nexus = NexusManager::new(project);
        nexus
        ...
        CloudConfig::Nexus(_) => {
            ...
            let build_mode_override = if dev {
                crate::output::warning(
                    "Using one-time dev build override for this deploy; sensibledb.toml build_mode is unchanged.",
                );
                Some(BuildMode::Dev)
            } else {
                None
...
fn parse_queries_for_metrics(project: &ProjectContext) -> Result<MetricsData> {
    use sensibledb_db::sensibledbc::parser::{
        NexusParser,
