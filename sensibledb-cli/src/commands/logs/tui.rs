...
/// Orange highlight color matching Nexus branding
const SENSIBLE_ORANGE: Color = Color::Rgb(253, 169, 66);
...
fn ui(f: &mut Frame, app: &App) {
    ...
    let header = Paragraph::new(header_text)
        .style(Style::default().fg(SENSIBLE_ORANGE))
        .block(Block::default().borders(Borders::ALL).title("Nexus Logs"));
...
fn format_picker_line(
    ...
    let base_style = if is_active {
        Style::default().fg(SENSIBLE_ORANGE)
    } else {
