...
const DEFAULT_CLOUD_AUTHORITY: &str = "cloud.sensibledb-db.com";
...
impl<'a> NexusManager<'a> {
    ...
    pub(crate) async fn deploy(
        ...
        let nexus_toml_path = path.join("sensibledb.toml");
                            ...
                            if io::stdin().read_line(&mut input).is_ok() {
                                ...
                                match choice {
                                    "1" | "" => {
                                        ...
                                        let comment = format!(
                                            "# SensibleDB Cloud URL for instance: {}",
                                            cluster_name
                                        );
                                        ...
                                        match crate::utils::add_env_var_to_file(
                                            ...
                                            Ok(_) => output::success(&format!(
                                                "Added SENSIBLE_CLOUD_URL and SENSIBLE_API_KEY to {}",
                                                env_path.display()
                                            )),
                                            Err(e) => {
                                                print_error(&format!("Failed to write .env: {}", e))
