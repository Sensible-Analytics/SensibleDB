...
impl<'a> FlyManager<'a> {
    ...
    pub async fn new(project: &'a ProjectContext, auth_type: FlyAuthType) -> Result<Self> {
        let auth = match auth_type {
            FlyAuthType::ApiKey => {
                let env_path = project.sensibledb_dir.join("sensibledb.env");
    ...
    fn app_name(&self, instance_name: &str) -> String {
        ...
        format!("nexus-{}-{}", sanitized_project_name, instance_name)
    }
    ...
    pub async fn init_app(&self, instance_name: &str, config: &FlyInstanceConfig) -> Result<()> {
        ...
        match &self.auth {
            ...
            FlyAuth::Cli => {
                ...
                let sensibledb_dir_path = self.project.instance_workspace(instance_name);
                ...
                let mut launch_args = vec![
                    ...
                    "--no-deploy",
                    "--path",
                    sensibledb_dir_path.to_str().ok_or_else(|| {
                        eyre!(
                            "cannot convert sensibledb instance workspace to string: {sensibledb_dir_path:?}"
                        )
                    })?,
                ];
    ...
    pub fn get_project_status(&self) -> Result<Vec<FlyAppStatus>> {
        ...
        let sanitized_project_name = self.project.config.project.name.replace('_', "-");
        let project_prefix = format!("nexus-{}-", sanitized_project_name);
