...
async fn run_add_inner(
            ...
            .contains_key(&instance_name)
    {
        return Err(project_error(format!(
            "Instance '{instance_name}' already exists in sensibledb.toml"
        ))
    ...
    let config_path = project_context.root.join("sensibledb.toml");
        ...
        CloudDeploymentTypeCommand::Nexus { .. } => {
            ...
            if flow_result.resolved_project_name != project_context.config.project.name {
                crate::output::info(&format!(
                    "Updating project name in sensibledb.toml to '{}' to match cloud project.",
            ...
            Step::verbose_substep("SensibleDB Cloud configuration saved");
        }
