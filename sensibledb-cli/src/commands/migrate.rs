use crate::config::{
    BuildMode, ContainerRuntime, DbConfig, GraphConfig, NexusConfig, LocalInstanceConfig,
...
pub async fn run(
    ...
    let nexus_toml_path = project_dir.join("sensibledb.toml");
    if nexus_toml_path.exists() {
        return Err(project_error("sensibledb.toml already exists in this directory")
...
fn detect_and_parse_v1_config(project_dir: &Path) -> Result<V1Config> {
    ...
    if !config_path.exists() {
        return Err(CliError::new("No config.hx.json file found")
            .with_hint("This doesn't appear to be a v1 Nexus project")
            .into());
    }
...
fn show_migration_plan(ctx: &MigrationContext) -> Result<()> {
    ...
    println!("  {}: sensibledb.toml", "Create file".bright_white().bold());

    if !ctx.no_backup {
        println!(
            "  {}: .sensibledb/v1-backup/config.hx.json",
    ...
    let v1_nexus_dir = home_dir.join(".sensibledb");
    if v1_nexus_dir.exists() {
        let v2_marker = v1_nexus_dir.join(".v2");
        if v2_marker.exists() {
            println!(
                "  {}: ~/.sensibledb directory already migrated to v2",
                "Already migrated".bright_white().bold()
            );
        } else {
            println!(
                "  {}: ~/.sensibledb → ~/.sensibledb-v1-backup",
                "Create backup".bright_white().bold()
            );
            if v1_nexus_dir.join("dockerdev").exists() {
                println!(
                    "  {}: Stop/remove nexus-dockerdev containers and images",
                    "Clean up Docker".bright_white().bold()
                );
            }
            println!(
                "  {}: Remove all except ~/.sensibledb/credentials and ~/.sensibledb/repo",
                "Clean directory".bright_white().bold()
            );
            if v1_nexus_dir.join("credentials").exists() {
                println!(
                    "  {}: ~/.sensibledb/credentials",
                    "Preserve file".bright_white().bold()
                );
            }
            if v1_nexus_dir.join("repo").exists() {
                println!(
                    "  {}: ~/.sensibledb/repo",
            ...
            println!(
                "  {}: Create ~/.sensibledb/.v2 marker file",
                "Mark migrated".bright_white().bold()
            );
        }
    } else {
        println!(
            "  {}: ~/.sensibledb directory not found",
...
fn create_backup(ctx: &MigrationContext) -> Result<()> {
    ...
    // Create .sensibledb/v1-backup directory
    let backup_dir = ctx.project_dir.join(".sensibledb/v1-backup");
    ...
    output::success("Created backup: .sensibledb/v1-backup/config.hx.json");
    Ok(())
}
...
fn create_v2_config(ctx: &MigrationContext) -> Result<()> {
    output::info("Creating sensibledb.toml configuration");
    ...
    // Save to file
    let config_path = ctx.project_dir.join("sensibledb.toml");
    nexus_config
        .save_to_file(&config_path)
        .map_err(|e| CliError::new("Failed to create sensibledb.toml").with_caused_by(e.to_string()))?;

    output::success("Created sensibledb.toml configuration");
    Ok(())
}

fn provide_post_migration_guidance(ctx: &MigrationContext) -> Result<()> {
    // Check if user has SensibleDB Cloud credentials
    ...
    print_instructions(
        ...
        &[
            &format!(
                "Run 'nexus check {}' to validate your configuration",
                ctx.instance_name
            ),
            &format!(
                "Run 'nexus push {}' to start your instance",
                ctx.instance_name
            ),
        ],
    );
    ...
    if has_cloud_credentials {
        output::info("You're authenticated with SensibleDB Cloud");
        output::info("The CLI v2 has enhanced cloud features with better instance management");
        print_instructions(
            "To set up cloud instances:",
            &[
                "Run 'nexus add cloud --name production' to add a production instance",
                "Run 'nexus add cloud --name staging' to add a staging instance",
                "Run 'nexus build production' to build for your cloud instance",
                "Run 'nexus push production' to deploy to SensibleDB Cloud",
            ],
        );
    } else {
        output::info("Ready for SensibleDB Cloud?");
        output::info("Take your project to production with managed infrastructure");
        print_instructions(
            "To get started with SensibleDB Cloud:",
            &[
                "Run 'nexus auth login' to authenticate with SensibleDB Cloud",
                "Run 'nexus add cloud --name production' to add a cloud instance",
                "Run 'nexus push production' to deploy to the cloud",
            ],
        );
    }
...
fn migrate_home_directory(_ctx: &MigrationContext) -> Result<()> {
    output::info("Migrating ~/.sensibledb directory");
    ...
    if !v1_nexus_dir.exists() {
        output::info("No ~/.sensibledb directory found, skipping home migration");
        return Ok(());
    }
    ...
    let v2_marker = v1_nexus_dir.join(".v2");
    if v2_marker.exists() {
        output::info("~/.sensibledb directory already migrated to v2, skipping home migration");
        return Ok(());
    }

    // Create backup of the entire .sensibledb directory
    let backup_dir = home_dir.join(".sensibledb-v1-backup");
    ...
    crate::utils::copy_dir_recursively(&v1_nexus_dir, &backup_dir).map_err(|e| {
        CliError::new("Failed to backup ~/.sensibledb directory").with_caused_by(e.to_string())
    })?;

    output::success("Created backup: ~/.sensibledb-v1-backup");

    // Clean up dockerdev containers/images if present
    let dockerdev_dir = v1_nexus_dir.join("dockerdev");
    if dockerdev_dir.exists() {
        cleanup_dockerdev()?;
    }

    // Remove everything except credentials and repo
    let credentials_path = v1_nexus_dir.join("credentials");
    let repo_path = v1_nexus_dir.join("repo");

    // Temporarily move credentials and repo out of the way
    let temp_credentials = if credentials_path.exists() {
        let temp_path = home_dir.join(".sensibledb-credentials-temp");
    ...
    let temp_repo = if repo_path.exists() {
        let temp_path = home_dir.join(".sensibledb-repo-temp");
    ...
    // Remove the entire .sensibledb directory
    fs::remove_dir_all(&v1_nexus_dir).map_err(|e| {
        CliError::new("Failed to remove ~/.sensibledb directory").with_caused_by(e.to_string())
    })?;

    // Recreate .sensibledb directory
    fs::create_dir_all(&v1_nexus_dir).map_err(|e| {
        CliError::new("Failed to recreate ~/.sensibledb directory").with_caused_by(e.to_string())
    })?;
    ...
    if let Some(temp_creds) = temp_credentials {
        ...
        output::info("Preserved ~/.sensibledb/credentials");
    }
    ...
    if let Some(temp_repo) = temp_repo {
        ...
        output::info("Preserved ~/.sensibledb/repo");
    }
    ...
    output::success("Cleaned up ~/.sensibledb directory, preserving credentials and repo");
    Ok(())
}
...
fn cleanup_dockerdev() -> Result<()> {
    ...
    let container_name = "nexus-dockerdev";
    ...
    // Try to remove any nexus-related images
...
fn check_cloud_credentials() -> bool {
    ...
    let credentials_path = home.join(".sensibledb").join("credentials");
