...
pub async fn run(
    ...
    let sensibledb_toml_path = project_dir.join("sensibledb.toml");
    if sensibledb_toml_path.exists() {
...
fn show_migration_plan(ctx: &MigrationContext) -> Result<()> {
    ...
    let v1_sensibledb_dir = home_dir.join(".sensibledb");
    if v1_sensibledb_dir.exists() {
        let v2_marker = v1_sensibledb_dir.join(".v2");
        if v2_marker.exists() {
            println!(
            ...
            if v1_sensibledb_dir.join("dockerdev").exists() {
                println!(
            ...
            println!(
                "  {}: Remove all except credentials and repo",
                "Clean directory".bright_white().bold()
            );
            if v1_sensibledb_dir.join("credentials").exists() {
                println!(
                    "  {}: credentials",
                    "Preserve file".bright_white().bold()
                );
            }
            if v1_sensibledb_dir.join("repo").exists() {
                println!(
                    "  {}: repo",
...
fn create_v2_config(ctx: &MigrationContext) -> Result<()> {
    ...
    let sensibledb_config = NexusConfig {
...
fn migrate_home_directory(_ctx: &MigrationContext) -> Result<()> {
    ...
    crate::utils::copy_dir_recursively(&v1_sensibledb_dir, &backup_dir).map_err(|e| {
    ...
    let dockerdev_dir = v1_sensibledb_dir.join("dockerdev");
    if dockerdev_dir.exists() {
        cleanup_dockerdev()?;
    }

    // Remove everything except credentials and repo
    let credentials_path = v1_sensibledb_dir.join("credentials");
    let repo_path = v1_sensibledb_dir.join("repo");
