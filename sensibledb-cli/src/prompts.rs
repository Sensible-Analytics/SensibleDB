//! Interactive prompts for the Nexus CLI using cliclack.
//!
...
/// AWS/SensibleDB Cloud region options
...
pub fn select_deployment_type() -> Result<DeploymentType> {
    let selected: DeploymentType = cliclack::select("Where would you like to deploy?")
        .item(
            ...
            "Local",
            "Run Nexus locally in Docker. Best for development.",
        )
        .item(
            DeploymentType::NexusCloud,
            "SensibleDB Cloud",
...
///
/// This is the main entry point for interactive mode for `nexus add`.
...
pub async fn build_deployment_command(
    ...
    // Check auth early for SensibleDB Cloud instances before prompting for more details
    if matches!(deployment_type, DeploymentType::NexusCloud) {
        require_auth().await?;
    }
    ...
    match deployment_type {
        ...
        DeploymentType::NexusCloud => {
            // Check auth early for SensibleDB Cloud instances
            let region = select_region()?;
            Ok(Some(CloudDeploymentTypeCommand::Nexus {
                region: Some(region),
                name: Some(instance_name),
            }))
        }
...
pub fn select_instance(instances: &[(&String, &str)]) -> Result<String> {
    if instances.is_empty() {
        return Err(eyre::eyre!(
            "No instances found in sensibledb.toml. Run 'sensibledb init' to create a project first."
        ));
    }
