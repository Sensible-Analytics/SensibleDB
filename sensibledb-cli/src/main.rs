...
#[derive(Subcommand)]
enum Commands {
    /// Initialize a new SensibleDB project with sensibledb.toml
    Init {
    ...
    /// Add a new instance to an existing SensibleDB project
    Add {
        #[command(subcommand)]
        cloud: Option<CloudDeploymentTypeCommand>,
    },
    ...
    Build {
        ...
        #[arg(short, long)]
        instance: Option<String>,
        /// Should build NexusDB into a binary at the specified directory location
        #[arg(long)]
        bin: Option<String>,
    },
...
fn display_welcome(update_available: Option<String>) {
    ...
    if let Ok(banner) = Banner::new("> NEXUS DB") {
        let banner = banner
