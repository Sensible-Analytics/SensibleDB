...
use eyre::Result;
use sensibledb_cli::{AuthAction, CloudDeploymentTypeCommand, DashboardAction, MetricsAction};
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize a new Nexus project with sensibledb.toml
    Init {
    ...
    /// Add a new instance to an existing Nexus project
    Add {
        #[command(subcommand)]
        cloud: Option<CloudDeploymentTypeCommand>,
    },
    ...
    Compile {
        /// Directory containing sensibledb.toml (defaults to current directory or project root)
        #[arg(short, long)]
        path: Option<String>,
    ...
    Build {
        ...
        #[arg(short, long)]
        instance: Option<String>,
        /// Should build SensibleDB into a binary at the specified directory location
        #[arg(long)]
        bin: Option<String>,
    },
    ...
    Push {
        ...
        instance: Option<String>,
        /// Use development profile for faster builds (SensibleDB Cloud only)
        #[arg(long)]
        dev: bool,
    },

    /// Sync .hx source files and config from a deployed SensibleDB Cloud instance
    Sync {
    ...
    /// Send feedback to the Nexus team
    Feedback {
...
fn display_welcome(update_available: Option<String>) {
    ...
    if let Ok(banner) = Banner::new("> SENSIBLE DB") {
        let banner = banner
    ...
    if let Some(latest_version) = update_available {
        if use_color {
            ...
            println!(
                "  │ Run '{}' to upgrade\n",
                "nexus update".truecolor(255, 165, 54).bold()
            );
        } else {
            println!("  | Update available: v{} ➜ v{}", version, latest_version);
            println!("  | Run 'nexus update' to upgrade\n");
        }
    }
    ...
    println!();
    print_command("nexus init", "Create a new Nexus project", use_color);
    print_command(
        "nexus init cloud",
        "Create a cloud-deployed project",
        use_color,
    );
    print_command("nexus build", "Build your project", use_color);
    print_command("nexus push", "Deploy/start an instance", use_color);
    ...
    println!();
    print_command("nexus status", "Show status of all instances", use_color);
    print_command("nexus logs", "View logs for an instance", use_color);
    print_command(
        "nexus dashboard start",
        "Launch the Nexus Dashboard",
        use_color,
    );
    print_command("nexus auth login", "Login to SensibleDB Cloud", use_color);
    ...
    println!();
    print_command("nexus --help", "Show all available commands", use_color);
    print_command(
        "nexus <command> --help",
    ...
    if use_color {
        println!(
            "  {} {}",
            "Docs:".dimmed(),
            "https://docs.sensibledb-db.com"
                .truecolor(253, 169, 66)
                .underline()
        );
    } else {
        println!("  Docs: https://docs.sensibledb-db.com");
    }
    println!();
}
