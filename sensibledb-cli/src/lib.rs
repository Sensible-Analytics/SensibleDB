// Library interface for sensibledb-cli to enable testing
use clap::Subcommand;
...
#[derive(Subcommand)]
pub enum AuthAction {
    /// Login to SensibleDB Cloud
    Login,
    /// Logout from SensibleDB Cloud
    Logout,
...
pub enum DashboardAction {
    /// Start the dashboard
    Start {
        /// Instance to connect to (from sensibledb.toml)
        ...
        /// SensibleDB host to connect to (e.g., localhost). Bypasses project config.
        #[arg(long)]
        host: Option<String>,

        /// SensibleDB port to connect to. Used with --host.
...
pub enum CloudDeploymentTypeCommand {
    /// Initialize SensibleDB Cloud deployment
    #[command(name = "cloud")]
    Nexus {
        /// Region for SensibleDB Cloud instance (default: us-east-1)
...
impl CloudDeploymentTypeCommand {
    pub fn name(&self) -> Option<String> {
        match self {
            CloudDeploymentTypeCommand::Nexus { name, .. } => name.clone(),
