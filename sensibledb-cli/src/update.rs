...
const GITHUB_API_URL: &str = "https://api.github.com/repos/sensibledb/sensibledb-db/releases/latest";
...
fn get_update_cache_path() -> Result<PathBuf> {
    ...
    // Ensure .sensibledb directory exists
    fs::create_dir_all(&nexus_dir)?;

    Ok(nexus_dir.join("update_cache.toml"))
}
