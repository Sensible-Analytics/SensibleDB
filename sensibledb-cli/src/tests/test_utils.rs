//! Test utilities for nexus-cli tests
//!
...
/// - A temporary cache directory (set via NEXUS_CACHE_DIR env var)
/// - A temporary nexus home directory (set via NEXUS_HOME env var)
///
/// The NEXUS_CACHE_DIR and NEXUS_HOME environment variables are automatically
...
pub struct TestContext {
    ...
    /// Guard to restore the NEXUS_CACHE_DIR env var on drop
    _cache_env_guard: EnvGuard,
    /// Guard to restore the NEXUS_HOME env var on drop
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        ...
        // depend on NEXUS_CACHE_DIR, but in practice each test gets its own
...
impl TestContext {
    ...
    /// 3. Set the NEXUS_CACHE_DIR environment variable to the cache directory
    /// 4. Set the NEXUS_HOME environment variable to the nexus home directory
    pub fn new() -> Self {
        ...
        let old_cache_value = std::env::var("NEXUS_CACHE_DIR").ok();
        let old_home_value = std::env::var("NEXUS_HOME").ok();
        ...
        unsafe {
            std::env::set_var("NEXUS_CACHE_DIR", &cache_dir);
    ...
    pub fn setup_valid_project(&self) {
        ...
        let config_path = self.project_path.join("nexus.toml");
        config
            .save_to_file(&config_path)
            .expect("Failed to save config");

        // Create .nexus directory
        fs::create_dir_all(self.project_path.join(".nexus")).expect("Failed to create .nexus");
...
mod tests {
    ...
    fn test_context_sets_env_var() {
        ...
        let env_value = std::env::var("NEXUS_CACHE_DIR").expect("NEXUS_CACHE_DIR should be set");
    ...
    // NEXUS_CACHE_DIR. The EnvGuard functionality is tested implicitly
    ...
    fn test_setup_valid_project() {
        ...
        assert!(ctx.project_path.join("nexus.toml").exists());
        assert!(ctx.project_path.join(".nexus").exists());
