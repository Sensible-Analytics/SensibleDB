//! Test utilities for sensibledb-cli tests
//!
...
/// - A temporary cache directory (set via SENSIBLE_CACHE_DIR env var)
/// - A temporary nexus home directory (set via SENSIBLE_HOME env var)
///
/// The SENSIBLE_CACHE_DIR and SENSIBLE_HOME environment variables are automatically
...
pub struct TestContext {
    ...
    /// The nexus home directory within the temp directory
    pub nexus_home: PathBuf,
    /// Guard to restore the SENSIBLE_CACHE_DIR env var on drop
    _cache_env_guard: EnvGuard,
    /// Guard to restore the SENSIBLE_HOME env var on drop
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        ...
        // depend on SENSIBLE_CACHE_DIR, but in practice each test gets its own
...
impl TestContext {
    ...
    /// 2. Create project, cache, and nexus home subdirectories
    /// 3. Set the SENSIBLE_CACHE_DIR environment variable to the cache directory
    /// 4. Set the SENSIBLE_HOME environment variable to the nexus home directory
    pub fn new() -> Self {
        let temp_dir = TempDir::new().expect("Failed to create temp dir");
        let project_path = temp_dir.path().join("project");
        let cache_dir = temp_dir.path().join("cache");
        let nexus_home = temp_dir.path().join(".sensibledb");

        std::fs::create_dir_all(&project_path).expect("Failed to create project dir");
        std::fs::create_dir_all(&cache_dir).expect("Failed to create cache dir");
        std::fs::create_dir_all(&nexus_home).expect("Failed to create nexus home dir");

        // Save old values and set new ones
        let old_cache_value = std::env::var("SENSIBLE_CACHE_DIR").ok();
        let old_home_value = std::env::var("SENSIBLE_HOME").ok();
        ...
        unsafe {
            std::env::set_var("SENSIBLE_CACHE_DIR", &cache_dir);
            std::env::set_var("SENSIBLE_HOME", &nexus_home);
        }
    ...
    /// Create a basic nexus project structure with valid schema and queries.
    ///
    /// This creates:
    /// - sensibledb.toml configuration file
    /// - .sensibledb directory
    ...
    pub fn setup_valid_project(&self) {
        ...
        // Create sensibledb.toml
        let config = NexusConfig::default_config("test-project");
        let config_path = self.project_path.join("sensibledb.toml");
        config
            .save_to_file(&config_path)
            .expect("Failed to save config");

        // Create .sensibledb directory
        fs::create_dir_all(self.project_path.join(".sensibledb")).expect("Failed to create .sensibledb");
...
"#;
    ...
    /// Create a nexus project with only schema (no queries).
...
"#;
    ...
    /// Create a nexus project without schema definitions (queries only, should fail validation).
...
"#;
    ...
    /// Create a nexus project with invalid syntax in queries.
...
QUERY InvalidQuery {
    this is not valid nexus syntax!!!
}
"#;
...
mod tests {
    ...
    fn test_context_sets_env_var() {
        ...
        let env_value = std::env::var("SENSIBLE_CACHE_DIR").expect("SENSIBLE_CACHE_DIR should be set");
    ...
    // SENSIBLE_CACHE_DIR. The EnvGuard functionality is tested implicitly
    ...
    fn test_setup_valid_project() {
        ...
        assert!(ctx.project_path.join("sensibledb.toml").exists());
        assert!(ctx.project_path.join(".sensibledb").exists());
