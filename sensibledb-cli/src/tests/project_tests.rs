...
fn setup_test_project() -> (TempDir, PathBuf) {
    ...
    fs::create_dir_all(project_path.join(".sensibledb")).expect("Failed to create .sensibledb");
...
fn test_project_context_instance_workspace() {
    ...
    assert_eq!(workspace, project_path.join(".sensibledb/dev"));
}
...
fn test_project_context_volumes_dir() {
    ...
    assert_eq!(volumes_dir, project_path.join(".sensibledb/.volumes"));
}
...
fn test_project_context_instance_volume() {
    ...
    assert_eq!(volume, project_path.join(".sensibledb/.volumes/production"));
}
...
fn test_project_context_docker_compose_path() {
    ...
    assert_eq!(
        compose_path,
        project_path.join(".sensibledb/staging/docker-compose.yml")
    );
}
...
fn test_project_context_container_dir() {
    ...
    assert_eq!(
        container_dir,
        project_path.join(".sensibledb/dev/sensibledb-container")
    );
}
...
fn test_get_nexus_cache_dir_creates_directory() {
    ...
    // When SENSIBLE_CACHE_DIR is set (by TestContext), get_nexus_cache_dir should use it
    ...
    // With SENSIBLE_CACHE_DIR override, the path should be the cache_dir from TestContext
    assert_eq!(
        cache_dir, ctx.cache_dir,
        "Cache directory should use SENSIBLE_CACHE_DIR when set"
    );
}
...
fn test_find_project_root_stops_at_filesystem_root() {
    ...
    // Start from a directory that definitely won't have sensibledb.toml
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
