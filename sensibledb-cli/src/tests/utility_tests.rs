...
async fn test_add_local_instance_succeeds() {
    ...
    let config_content =
        fs::read_to_string(ctx.project_path.join("sensibledb.toml")).expect("Should read config");
    assert!(
...
async fn test_migrate_fails_if_v2_exists() {
    ...
    // Also create sensibledb.toml (v2 marker)
    fs::write(
        ctx.project_path.join("sensibledb.toml"),
        "[project]\nname = \"test\"",
    )
    .expect("Failed to write sensibledb.toml");
    ...
    assert!(result.is_err(), "Migrate should fail if sensibledb.toml exists");
    let error_msg = format!("{:?}", result.err().unwrap());
    assert!(
        error_msg.contains("sensibledb.toml") || error_msg.contains("v2"),
...
async fn test_migrate_dry_run_shows_plan() {
    ...
    assert!(
        !ctx.project_path.join("sensibledb.toml").exists(),
        "sensibledb.toml should NOT be created during dry run"
    );
}
