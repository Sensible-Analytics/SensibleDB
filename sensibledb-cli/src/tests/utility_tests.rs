...
#[tokio::test]
async fn test_add_fails_without_nexus_project() {
    ...
    assert!(
        result.is_err(),
        "Add should fail when not in a nexus project"
    );
}
...
async fn test_add_local_instance_succeeds() {
    ...
    let config_content =
        fs::read_to_string(ctx.project_path.join("nexus.toml")).expect("Should read config");
    assert!(
...
async fn test_prune_fails_for_specific_instance_outside_project() {
    ...
    let error_msg = format!("{:?}", result.err().unwrap());
    assert!(
        error_msg.contains("not in") || error_msg.contains("Nexus project"),
        "Error should mention not in nexus project: {error_msg}"
    );
}
...
async fn test_metrics_basic_enables_collection() {
    ...
    // (avoids race conditions with NEXUS_HOME env var in parallel tests)
    let config_path = ctx.nexus_home.join("metrics.toml");
    assert!(
        config_path.exists(),
...
async fn test_migrate_fails_if_v2_exists() {
    ...
    // Also create nexus.toml (v2 marker)
    fs::write(
        ctx.project_path.join("nexus.toml"),
        "[project]\nname = \"test\"",
    )
    .expect("Failed to write nexus.toml");
    ...
    assert!(result.is_err(), "Migrate should fail if nexus.toml exists");
    let error_msg = format!("{:?}", result.err().unwrap());
    assert!(
        error_msg.contains("nexus.toml") || error_msg.contains("v2"),
...
async fn test_migrate_dry_run_shows_plan() {
    ...
    assert!(
        !ctx.project_path.join("nexus.toml").exists(),
        "nexus.toml should NOT be created during dry run"
    );
}
...
"#;
    ...
    let result: Result<NexusConfig, _> = toml::from_str(config_content);
...
fn test_config_validates_build_mode_debug_is_rejected() {
    ...
    // (validation happens in NexusConfig::from_file)
    let config_content = r#"
[project]
name = "test"
queries = "./db/"
...
"#;
    ...
    let config: NexusConfig = toml::from_str(config_content).expect("Should parse");
...
fn test_config_default_has_dev_instance() {
    let config = NexusConfig::default_config("my-project");
...
fn test_config_list_instances() {
    let mut config = NexusConfig::default_config("my-project");
