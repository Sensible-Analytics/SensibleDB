...
use crate::config::{DbConfig, NexusConfig, LocalInstanceConfig};
...
/// This test requires cloning the nexus repo which is slow and requires network.
/// Run with: cargo test --package sensibledb-cli -- --ignored --test-threads=1
#[tokio::test]
#[ignore]
...
#[tokio::test]
async fn test_check_fails_without_nexus_toml() {
    let ctx = TestContext::new();
    ...
    assert!(
        result.is_err(),
        "Check should fail without sensibledb.toml in project"
    );
    let error_msg = result.err().unwrap().to_string();
    assert!(
        error_msg.contains("not found") || error_msg.contains("sensibledb.toml"),
        "Error should mention missing sensibledb.toml"
    );
}
...
async fn test_check_with_multiple_instances() {
    ...
    // Create sensibledb.toml with multiple instances
    ...
    let config_path = ctx.project_path.join("sensibledb.toml");
    config
        .save_to_file(&config_path)
        .expect("Failed to save config");

    // Create .sensibledb directory
    fs::create_dir_all(ctx.project_path.join(".sensibledb")).expect("Failed to create .sensibledb");
...
async fn test_check_with_custom_queries_path() {
    ...
    // Create sensibledb.toml with custom queries path
    let mut config = NexusConfig::default_config("test-project");
    config.project.queries = PathBuf::from("custom/nexus/queries");
    let config_path = ctx.project_path.join("sensibledb.toml");
    config
        .save_to_file(&config_path)
        .expect("Failed to save config");
    ...
    let queries_dir = ctx.project_path.join("custom/nexus/queries");
