...
/// Run with: cargo test --package nexus-cli -- --ignored --test-threads=1
#[tokio::test]
#[ignore]
...
async fn test_check_fails_without_nexus_toml() {
    ...
    assert!(
        result.is_err(),
        "Check should fail without nexus.toml in project"
    );
    let error_msg = result.err().unwrap().to_string();
    assert!(
        error_msg.contains("not found") || error_msg.contains("nexus.toml"),
        "Error should mention missing nexus.toml"
    );
}
...
async fn test_check_with_multiple_instances() {
    ...
    // Create nexus.toml with multiple instances
    ...
    let config_path = ctx.project_path.join("nexus.toml");
    config
...
async fn test_check_with_custom_queries_path() {
    ...
    // Create nexus.toml with custom queries path
    let mut config = NexusConfig::default_config("test-project");
