...
/// Regression test: SENSIBLE_DATA_DIR must always be /data inside container
/// (Bug from PR #823 where host path was incorrectly passed to container)
#[test]
fn test_nexus_data_dir_uses_container_path() {
    ...
    let data_dir_var = env_vars
        .iter()
        .find(|v| v.starts_with("SENSIBLE_DATA_DIR="))
        .expect("SENSIBLE_DATA_DIR should be set");

    assert_eq!(
        data_dir_var, "SENSIBLE_DATA_DIR=/data",
        "SENSIBLE_DATA_DIR must use container path /data, not host path"
    );
}
...
fn test_docker_compose_volume_and_env() {
    ...
    assert!(
        compose.contains("SENSIBLE_DATA_DIR=/data"),
        "SENSIBLE_DATA_DIR in environment should be /data"
    );

    // Should NOT have host path in SENSIBLE_DATA_DIR
    assert!(
        !compose.contains("SENSIBLE_DATA_DIR=../.volumes"),
        "SENSIBLE_DATA_DIR should NOT contain host path"
    );
}

/// Verify host-side data_dir() still respects SENSIBLE_DATA_DIR for volume mount
#[test]
