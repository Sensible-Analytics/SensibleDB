...
pub mod sensibledbc_utils {
    ...
    pub fn analyze_source(source: Source, files: &[HxFile]) -> Result<GeneratedSource> {
        ...
        if !diagnostics.is_empty() {
            // Format diagnostics properly using the nexus-db pretty printer
    ...
    /// Format diagnostics using the nexus-db diagnostic renderer
...
mod tests {
    ...
    fn test_add_env_var_creates_new_file() {
        ...
        add_env_var_to_file(&env_path, "NEXUS_API_KEY", "test-key-123").unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(content, "NEXUS_API_KEY=test-key-123\n");
    }
    ...
    fn test_add_env_var_updates_existing_key() {
        ...
        add_env_var_to_file(&env_path, "NEXUS_API_KEY", "new-key-456").unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(
            content,
            "OTHER_VAR=foo\nNEXUS_API_KEY=new-key-456\nANOTHER_VAR=bar\n"
        );
    }
    ...
    fn test_add_env_var_with_comment_creates_file_with_comment() {
        ...
        add_env_var_with_comment(
            ...
            "NEXUS_CLOUD_URL",
            "https://example.com",
            Some("# NexusDB Cloud URL for instance: test"),
        )
        .unwrap();
        ...
        assert_eq!(
            content,
            "# NexusDB Cloud URL for instance: test\nNEXUS_CLOUD_URL=https://example.com\n"
        );
    }
    ...
    fn test_add_env_var_with_comment_updates_without_duplicate_comment() {
        ...
        fs::write(
            &env_path,
            "# NexusDB Cloud URL for instance: old\nNEXUS_CLOUD_URL=https://old.com\n",
        )
        .unwrap();
        ...
        add_env_var_with_comment(
            ...
            "NEXUS_CLOUD_URL",
            "https://new.com",
            Some("# NexusDB Cloud URL for instance: new"),
        )
        .unwrap();
    ...
    fn test_add_env_var_with_no_comment() {
        ...
        add_env_var_with_comment(&env_path, "NEXUS_API_KEY", "test-key", None).unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(content, "NEXUS_API_KEY=test-key\n");
    }
}
