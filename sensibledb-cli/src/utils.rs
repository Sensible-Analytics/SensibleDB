...
use sensibledb_db::sensibledbc::parser::types::HxFile;
use std::io::IsTerminal;
use std::{borrow::Cow, fs, path::Path};
use tokio::sync::oneshot;
use tokio::time::Duration;

const IGNORES: [&str; 3] = ["target", ".git", ".sensibledb"];
...
pub mod sensibledbc_utils {
    use eyre::Result;
    use sensibledb_db::sensibledbc::{
        analyzer::analyze,
    ...
    pub fn collect_hx_files(root: &Path, queries_dir: &Path) -> Result<Vec<std::fs::DirEntry>> {
        ...
        fn collect_from_dir(dir: &Path, files: &mut Vec<std::fs::DirEntry>) -> Result<()> {
            if dir.file_name().unwrap_or_default() == ".sensibledb" {
                return Ok(());
            }
    ...
    pub fn parse_content(content: &Content) -> Result<Source> {
        let source =
            NexusParser::parse_source(content).map_err(|e| eyre::eyre!("Parse error: {}", e))?;
        Ok(source)
    }
    ...
    pub fn analyze_source(source: Source, files: &[HxFile]) -> Result<GeneratedSource> {
        ...
        if !diagnostics.is_empty() {
            // Format diagnostics properly using the sensibledb-db pretty printer
    ...
    /// Format diagnostics using the sensibledb-db diagnostic renderer
    fn format_diagnostics(
        diagnostics: &[sensibledb_db::sensibledbc::analyzer::diagnostic::Diagnostic],
        src: &str,
        files: &[HxFile],
    ) -> String {
...
mod tests {
    ...
    fn test_add_env_var_creates_new_file() {
        ...
        add_env_var_to_file(&env_path, "SENSIBLE_API_KEY", "test-key-123").unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(content, "SENSIBLE_API_KEY=test-key-123\n");
    }
    ...
    fn test_add_env_var_updates_existing_key() {
        ...
        fs::write(
            &env_path,
            "OTHER_VAR=foo\nSENSIBLE_API_KEY=old-key\nANOTHER_VAR=bar\n",
        )
        .unwrap();

        add_env_var_to_file(&env_path, "SENSIBLE_API_KEY", "new-key-456").unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(
            content,
            "OTHER_VAR=foo\nSENSIBLE_API_KEY=new-key-456\nANOTHER_VAR=bar\n"
        );
    }
    ...
    fn test_add_env_var_preserves_other_variables() {
        ...
        add_env_var_to_file(&env_path, "SENSIBLE_API_KEY", "my-key").unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert!(content.contains("VAR1=value1"));
        assert!(content.contains("VAR2=value2"));
        assert!(content.contains("VAR3=value3"));
        assert!(content.contains("SENSIBLE_API_KEY=my-key"));
    }
    ...
    fn test_add_env_var_with_comment_creates_file_with_comment() {
        ...
        add_env_var_with_comment(
            ...
            "SENSIBLE_CLOUD_URL",
            "https://example.com",
            Some("# SensibleDB Cloud URL for instance: test"),
        )
        .unwrap();
        ...
        assert_eq!(
            content,
            "# SensibleDB Cloud URL for instance: test\nSENSIBLE_CLOUD_URL=https://example.com\n"
        );
    }
    ...
    fn test_add_env_var_with_comment_updates_without_duplicate_comment() {
        ...
        fs::write(
            &env_path,
            "# SensibleDB Cloud URL for instance: old\nSENSIBLE_CLOUD_URL=https://old.com\n",
        )
        .unwrap();
        ...
        add_env_var_with_comment(
            ...
            "SENSIBLE_CLOUD_URL",
            "https://new.com",
            Some("# SensibleDB Cloud URL for instance: new"),
        )
        .unwrap();
        ...
        assert_eq!(
            content,
            "# SensibleDB Cloud URL for instance: old\nSENSIBLE_CLOUD_URL=https://new.com\n"
        );
    }
    ...
    fn test_add_env_var_with_no_comment() {
        ...
        add_env_var_with_comment(&env_path, "SENSIBLE_API_KEY", "test-key", None).unwrap();

        let content = fs::read_to_string(&env_path).unwrap();
        assert_eq!(content, "SENSIBLE_API_KEY=test-key\n");
    }
}
