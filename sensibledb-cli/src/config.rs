...
use sensibledb_db::sensibledb_engine::{
...
/// Global workspace configuration stored in ~/.sensibledb/config
...
impl WorkspaceConfig {
    ...
    pub fn config_path() -> Result<PathBuf, ConfigError> {
        ...
        Ok(home.join(".sensibledb").join("config"))
    }

    /// Load the workspace config from ~/.sensibledb/config
    ...
    /// Save the workspace config to ~/.sensibledb/config
...
impl CloudConfig {
    pub fn get_cluster_id(&self) -> Option<&str> {
        match self {
            CloudConfig::Nexus(config) => Some(&config.cluster_id),
    ...
    pub fn build_mode(&self) -> BuildMode {
        match self {
            Self::Nexus(CloudInstanceConfig { build_mode, .. })
...
impl<'a> InstanceInfo<'a> {
    pub fn build_mode(&self) -> BuildMode {
        match self {
            ...
            | InstanceInfo::Nexus(CloudInstanceConfig { build_mode, .. })
    ...
    pub fn cluster_id(&self) -> Option<&str> {
        match self {
            ...
            InstanceInfo::Nexus(config) => Some(&config.cluster_id),
    ...
    pub fn db_config(&self) -> &DbConfig {
        match self {
            ...
            | InstanceInfo::Nexus(CloudInstanceConfig { db_config, .. })
    ...
    pub fn docker_build_target(&self) -> Option<&str> {
        match self {
            InstanceInfo::Local(_) | InstanceInfo::Nexus(_) | InstanceInfo::Enterprise(_) => None,
...
impl From<InstanceInfo<'_>> for CloudConfig {
    fn from(instance_info: InstanceInfo<'_>) -> Self {
        match instance_info {
            InstanceInfo::Nexus(config) => CloudConfig::Nexus(config.clone()),
...
impl NexusConfig {
    pub fn from_file(path: &Path) -> Result<Self, ConfigError> {
        let content = fs::read_to_string(path).map_err(|source| ConfigError::ReadNexusConfig {
            path: path.to_path_buf(),
            source,
        })?;

        let config: NexusConfig =
            toml::from_str(&content).map_err(|source| ConfigError::ParseNexusConfig {
                path: path.to_path_buf(),
                source,
            })?;
    ...
    pub fn save_to_file(&self, path: &Path) -> Result<(), ConfigError> {
        let content = toml::to_string_pretty(self)
            .map_err(|source| ConfigError::SerializeNexusConfig { source })?;

        fs::write(path, content).map_err(|source| ConfigError::WriteNexusConfig {
            path: path.to_path_buf(),
            source,
        })?;
    ...
    pub fn get_instance(&self, name: &str) -> Result<InstanceInfo<'_>, ConfigError> {
        ...
        if let Some(cloud_config) = self.cloud.get(name) {
            match cloud_config {
                CloudConfig::Nexus(config) => {
                    return Ok(InstanceInfo::Nexus(config));
                }
    ...
    /// Returns tuples of (name, type_hint) e.g. ("dev", "local"), ("prod", "SensibleDB Cloud")
    pub fn list_instances_with_types(&self) -> Vec<(&String, &'static str)> {
        ...
        for (name, config) in &self.cloud {
            let type_hint = match config {
                CloudConfig::Nexus(_) => "SensibleDB Cloud",
