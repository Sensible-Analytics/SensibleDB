...
impl ProjectContext {
    ...
    pub fn find_and_load(start_dir: Option<&Path>) -> Result<Self, ProjectError> {
        ...
        let sensibledb_dir = root.join(".sensibledb");
    ...
    pub fn instance_workspace(&self, instance_name: &str) -> PathBuf {
        self.sensibledb_dir.join(instance_name)
    }

    /// Get the volumes directory for persistent data
    pub fn volumes_dir(&self) -> PathBuf {
        self.sensibledb_dir.join(".volumes")
    }
...
pub fn get_sensibledb_cache_dir() -> Result<PathBuf> {
    if let Ok(override_dir) = std::env::var("SENSIBLEDB_CACHE_DIR") {
        let sensibledb_dir = PathBuf::from(override_dir);
        std::fs::create_dir_all(&sensibledb_dir)?;
        return Ok(sensibledb_dir);
    }

    let home = dirs::home_dir().ok_or_else(|| eyre!("Cannot find home directory"))?;
    let sensibledb_dir = home.join(".sensibledb");

    let is_fresh_install = !sensibledb_dir.exists();

    std::fs::create_dir_all(&sensibledb_dir)?;

    if is_fresh_install {
        let v2_marker = sensibledb_dir.join(".v2");
...
pub fn get_sensibledb_repo_cache() -> Result<PathBuf> {
    let sensibledb_dir = get_sensibledb_cache_dir()?;
    Ok(sensibledb_dir.join("repo"))
}
