...
pub enum ConfigError {
    ...
    #[error("failed to read sensibledb.toml at {path}: {source}")]
    ReadNexusConfig {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to parse sensibledb.toml at {path}: {source}")]
    ParseNexusConfig {
        path: PathBuf,
        #[source]
        source: toml::de::Error,
    },
    #[error("failed to serialize sensibledb.toml: {source}")]
    SerializeNexusConfig {
        #[source]
        source: toml::ser::Error,
    },
    #[error("failed to write sensibledb.toml at {path}: {source}")]
    WriteNexusConfig {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("instance '{name}' not found in sensibledb.toml")]
...
pub enum ProjectError {
    ...
    #[error(
        "found v1 project configuration at {path}; run 'nexus migrate --path \"{root}\"' to migrate"
    )]
...
impl ConfigError {
    pub fn to_cli_error(&self) -> CliError {
        match self {
            ...
            .with_caused_by(source.to_string()),
            ConfigError::ReadNexusConfig { path, source } => {
                CliError::new(format!("failed to read sensibledb.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ConfigError::ParseNexusConfig { path, source } => {
                CliError::new(format!("failed to parse sensibledb.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ConfigError::SerializeNexusConfig { source } => {
                CliError::new("failed to serialize sensibledb.toml").with_caused_by(source.to_string())
            }
            ConfigError::WriteNexusConfig { path, source } => {
                CliError::new(format!("failed to write sensibledb.toml at {}", path.display()))
                    .with_caused_by(source.to_string())
            }
            ...
            ConfigError::InstanceNotFound { name } => {
                CliError::new(format!("instance '{}' not found in sensibledb.toml", name))
            }
        }
    }
}
        ...
        match self {
            ...
            ProjectError::LegacyConfig { path, root } => {
                config_error("found v1 project configuration")
                    ...
                    .with_hint(format!(
                        "Run 'nexus migrate --path \"{}\"' to migrate this project to v2 format",
                        root.display()
                    ))
            }
...
impl From<toml::de::Error> for CliError {
    fn from(err: toml::de::Error) -> Self {
        CliError::new("failed to parse TOML configuration")
            .with_caused_by(err.to_string())
            .with_hint("check the sensibledb.toml file for syntax errors")
    }
}
...
pub fn config_error<S: Into<String>>(message: S) -> CliError {
    CliError::new(message).with_hint("run `nexus init` if you need to create a new project")
}
...
pub fn project_error<S: Into<String>>(message: S) -> CliError {
    CliError::new(message).with_hint("ensure you're in a valid nexus project directory")
}

#[allow(unused)]
pub fn cloud_error<S: Into<String>>(message: S) -> CliError {
    CliError::new(message).with_hint("run `nexus auth login` to authenticate with SensibleDB Cloud")
}
