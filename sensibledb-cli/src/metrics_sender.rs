...
const METRICS_URL: &str = "https://logs.sensibledb-db.com/v2";
...
pub(crate) fn get_metrics_config_path() -> Result<PathBuf> {
    let nexus_dir = if let Ok(home) = std::env::var("SENSIBLE_HOME") {
        PathBuf::from(home)
    } else {
    ...
    Ok(nexus_dir.join("metrics.toml"))
}
...
fn get_metrics_dir() -> Result<PathBuf> {
    ...
    let metrics_dir = nexus_dir.join("metrics");
