...
const METRICS_URL: &str = "https://logs.sensibledb.com/v2";
...
pub(crate) fn get_metrics_config_path() -> Result<PathBuf> {
    let sensibledb_dir = if let Ok(home) = std::env::var("SENSIBLEDB_HOME") {
        PathBuf::from(home)
    } else {
        let home = home_dir().ok_or_eyre("Cannot find home directory")?;
        home.join(".sensibledb")
    };
    fs::create_dir_all(&sensibledb_dir)?;
    Ok(sensibledb_dir.join("metrics.toml"))
}
...
fn get_metrics_dir() -> Result<PathBuf> {
    ...
    let metrics_dir = sensibledb_dir.join("metrics");
