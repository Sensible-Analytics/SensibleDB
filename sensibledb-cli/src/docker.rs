...
impl<'a> DockerManager<'a> {
    ...
    pub(crate) fn data_dir(&self, instance_name: &str) -> String {
        std::env::var("SENSIBLE_DATA_DIR").unwrap_or_else(|_| format!("../.volumes/{instance_name}"))
    }
        ...
        let mut env_vars = vec![
            {
                let port = self
                    ...
                    .port()
                    .unwrap_or(6969);
                format!("SENSIBLE_PORT={port}")
            },
            format!("SENSIBLE_DATA_DIR=/data"),
            format!("SENSIBLE_INSTANCE={instance_name}"),
            {
                let project_name = &self.project.config.project.name;
                format!("SENSIBLE_PROJECT={project_name}")
            },
        ];
        if let Ok(core_override) = std::env::var("SENSIBLE_CORES_OVERRIDE") {
            env_vars.push(format!("SENSIBLE_CORES_OVERRIDE={core_override}"));
        }
    ...
    pub fn generate_dockerfile(
        ...
        let dockerfile = format!(
            r#"# Generated Dockerfile for Nexus instance: {instance_name}
...
COPY nexus-repo-copy/ ./

# Then overlay instance-specific files
COPY sensibledb-container/ ./sensibledb-container/

FROM chef AS planner
# Generate the recipe file for dependency caching
RUN cargo chef prepare --recipe-path recipe.json --bin sensibledb-container
...
RUN cargo chef cook {build_flag} --recipe-path recipe.json --bin sensibledb-container

# Copy source code and build the application
COPY nexus-repo-copy/ ./
COPY sensibledb-container/ ./sensibledb-container/
RUN cargo build {build_flag} --package sensibledb-container
...
COPY --from=builder /build/target/{build_mode}/sensibledb-container /usr/local/bin/sensibledb-container
...
"#
    ...
    pub fn generate_docker_compose(
        ...
        let compose = format!(
            r#"# Generated docker-compose.yml for Nexus instance: {instance_name}
services:
  {service_name}:
    build:
      context: .
...
"#,
    ...
    pub fn get_project_status(&self) -> Result<Vec<ContainerStatus>> {
        let project_name = &self.project.config.project.name;
        let filter = format!("name=nexus-{project_name}-");
        ...
        for line in stdout.lines() {
            ...
            if parts.len() >= 3 {
                ...
                // Extract instance name from new container naming scheme: nexus-{project}-{instance}-app
                let expected_prefix = format!("nexus-{project_name}-");
    ...
    pub fn prune_instance(&self, instance_name: &str, remove_volumes: bool) -> Result<()> {
        ...
        // Old volume naming pattern: nexus_{project_name}_{instance_name}_data
        if remove_volumes {
    ...
    /// Get all Nexus-related images from the system
    pub fn get_nexus_images(runtime: ContainerRuntime) -> Result<Vec<String>> {
    ...
    /// Remove all Nexus-related images from the system
    pub fn clean_all_nexus_images(runtime: ContainerRuntime) -> Result<()> {
        Step::verbose_substep(&format!(
            "{}: Finding all Nexus images on system...",
            runtime.label()
        ));

        let images = Self::get_nexus_images(runtime)?;

        if images.is_empty() {
            Step::verbose_substep(&format!(
                "{}: No Nexus images found to clean",
                runtime.label()
            ));
            return Ok(());
        }
        ...
        Step::verbose_substep(&format!(
            "{}: Found {count} Nexus images to remove",
            runtime.label()
        ));
