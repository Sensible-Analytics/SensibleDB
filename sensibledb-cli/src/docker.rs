...
impl<'a> DockerManager<'a> {
    ...
    pub(crate) fn data_dir(&self, instance_name: &str) -> String {
        std::env::var("NEXUS_DATA_DIR").unwrap_or_else(|_| format!("../.volumes/{instance_name}"))
    }
        ...
        let mut env_vars = vec![
            {
                let port = self
                    ...
                    .port()
                    .unwrap_or(6969);
                format!("NEXUS_PORT={port}")
            },
            format!("NEXUS_DATA_DIR=/data"),
            format!("NEXUS_INSTANCE={instance_name}"),
            {
                let project_name = &self.project.config.project.name;
                format!("NEXUS_PROJECT={project_name}")
            },
        ];
        if let Ok(core_override) = std::env::var("NEXUS_CORES_OVERRIDE") {
            env_vars.push(format!("NEXUS_CORES_OVERRIDE={core_override}"));
        }
...
COPY nexus-container/ ./nexus-container/

FROM chef AS planner
# Generate the recipe file for dependency caching
RUN cargo chef prepare --recipe-path recipe.json --bin nexus-container
...
RUN cargo chef cook {build_flag} --recipe-path recipe.json --bin nexus-container

# Copy source code and build the application
COPY nexus-repo-copy/ ./
COPY nexus-container/ ./nexus-container/
RUN cargo build {build_flag} --package nexus-container
...
COPY --from=builder /build/target/{build_mode}/nexus-container /usr/local/bin/nexus-container
...
"#,
    ...
    pub fn get_sensibledb_images(runtime: ContainerRuntime) -> Result<Vec<String>> {
    ...
    pub fn clean_all_sensibledb_images(runtime: ContainerRuntime) -> Result<()> {
        Step::verbose_substep(&format!(
            "{}: Finding all Nexus images on system...",
            runtime.label()
        ));

        let images = Self::get_sensibledb_images(runtime)?;
