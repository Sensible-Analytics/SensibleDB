...
fn main() {
    ...
    let path = match std::env::var("SENSIBLE_DATA_DIR") {
        Ok(val) => std::path::PathBuf::from(val).join("user"),
        Err(_) => {
            println!("SENSIBLE_DATA_DIR not set, using default");
            let home = dirs::home_dir().expect("Could not retrieve home directory");
            home.join(".sensibledb/user")
        }
    };

    let port = match std::env::var("SENSIBLE_PORT") {
        Ok(val) => val
            .parse::<u16>()
            .expect("SENSIBLE_PORT must be a valid port number"),
        Err(_) => 6969,
    };
